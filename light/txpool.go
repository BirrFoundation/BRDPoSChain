...
func (p *TxPool) validateTx(ctx context.Context, tx *types.Transaction) error {
	...
	if tx.IsBRCZApplyTransaction() {
		...
		if err := core.ValidateBRCZApplyTransaction(p.chain, nil, copyState, common.BytesToAddress(tx.Data()[4:])); err != nil {
			return err
		}
	}
	...
	if tx.IsBRCXApplyTransaction() {
		copyState := p.currentState(ctx).Copy()
		if err := core.ValidateBRCXApplyTransaction(p.chain, nil, copyState, common.BytesToAddress(tx.Data()[4:])); err != nil {
			return err
		}
	}
