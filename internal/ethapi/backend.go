...
type Backend interface {
	...
	BRCxService() *BRCx.BRCX
	LendingService() *BRCxlending.Lending
	...
	GetEVM(ctx context.Context, msg core.Message, state *state.StateDB, BRCxState *tradingstate.TradingStateDB, header *types.Header, vmConfig *vm.Config) (*vm.EVM, func() error, error)
	...
	return []rpc.API{
		{
			...
			Namespace: "BRCx",
			Version:   "1.0",
			Service:   NewPublicBRCXTransactionPoolAPI(apiBackend, nonceLock),
			Public:    true,
		}, {
			Namespace: "txpool",
			Version:   "1.0",
