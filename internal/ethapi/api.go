...
import (
	...
	"BRDPoSChain/common/sort"
	"BRDPoSChain/consensus"
	"BRDPoSChain/consensus/BRDPoS"
	"BRDPoSChain/consensus/BRDPoS/utils"
	"BRDPoSChain/consensus/ethash"
	"BRDPoSChain/consensus/misc/eip1559"
	contractValidator "BRDPoSChain/contracts/validator/contract"
	"BRDPoSChain/core"
...
//
// https://BRDPoSChain/wiki/Management-APIs#personal_sign
...
//
// https://BRDPoSChain/wiki/Management-APIs#personal_ecRecover
...
func (s *PublicBlockChainAPI) GetCandidateStatus(ctx context.Context, coinbaseAddress common.Address, epoch rpc.EpochNumber) (map[string]interface{}, error) {
	...
	epochConfig := s.b.ChainConfig().BRDPoS.Epoch
	...
	if header.Number.Cmp(s.b.ChainConfig().BRDPoS.V2.SwitchBlock) == 1 {
		if engine, ok := s.b.GetEngine().(*BRDPoS.BRDPoS); ok {
			round, err := engine.EngineV2.GetRoundNumber(header)
			if err != nil {
				return result, err
			}
			maxMasternodes = s.b.ChainConfig().BRDPoS.V2.Config(uint64(round)).MaxMasternodes
		} else {
			return result, errors.New("undefined BRDPoS consensus engine")
		}
...
func (s *PublicBlockChainAPI) GetCheckpointFromEpoch(ctx context.Context, epochNum rpc.EpochNumber) (rpc.BlockNumber, rpc.EpochNumber) {
	...
	epoch := s.b.ChainConfig().BRDPoS.Epoch
...
func (s *PublicBlockChainAPI) getCandidatesFromSmartContract() ([]utils.Masternode, error) {
	...
	validator, err := contractValidator.NewBRCValidator(addr, client)
	if err != nil {
		return []utils.Masternode{}, err
	}
...
func DoCall(ctx context.Context, b Backend, args TransactionArgs, blockNrOrHash rpc.BlockNumberOrHash, overrides *StateOverride, timeout time.Duration, globalGasCap uint64) (*core.ExecutionResult, error) {
	...
	BRCxState, err := b.BRCxService().GetTradingState(block, author)
	if err != nil {
		return nil, err
	}
	...
	evm, vmError, err := b.GetEVM(ctx, msg, statedb, BRCxState, header, &vm.Config{NoBaseFee: true})
	if err != nil {
		return nil, err
	}
...
func (s *PublicBlockChainAPI) findNearestSignedBlock(ctx context.Context, b *types.Block) *types.Block {
	...
	// Get block epoc latest
	checkpointNumber, _, err := s.b.GetEngine().(*BRDPoS.BRDPoS).GetCurrentEpochSwitchBlock(s.chainReader, big.NewInt(int64(signedBlockNumber)))
	if err != nil {
...
func (s *PublicBlockChainAPI) findFinalityOfBlock(ctx context.Context, b *types.Block, masternodes []common.Address) (uint, error) {
	engine, _ := s.b.GetEngine().(*BRDPoS.BRDPoS)
...
*/
func (s *PublicBlockChainAPI) getSigners(ctx context.Context, block *types.Block, engine *BRDPoS.BRDPoS) ([]common.Address, error) {
	var err error
...
func (s *PublicBlockChainAPI) rpcOutputBlockSigners(b *types.Block, ctx context.Context, masternodes []common.Address) ([]common.Address, error) {
	...
	engine, ok := s.b.GetEngine().(*BRDPoS.BRDPoS)
	if !ok {
		log.Error("Undefined BRDPoS consensus engine")
...
func AccessList(ctx context.Context, b Backend, blockNrOrHash rpc.BlockNumberOrHash, args TransactionArgs) (acl types.AccessList, gasUsed uint64, vmErr error, err error) {
	...
	for {
		...
		vmenv, _, err := b.GetEVM(ctx, msg, statedb, BRCxState, header, &config)
		if err != nil {
			return nil, 0, nil, err
		}
...
// PublicTransactionPoolAPI exposes methods for the RPC interface
type PublicBRCXTransactionPoolAPI struct {
	b         Backend
	nonceLock *AddrLocker
}
...
// NewPublicTransactionPoolAPI creates a new RPC service with methods specific for the transaction pool.
func NewPublicBRCXTransactionPoolAPI(b Backend, nonceLock *AddrLocker) *PublicBRCXTransactionPoolAPI {
	return &PublicBRCXTransactionPoolAPI{b, nonceLock}
}
...
// The sender is responsible for signing the transaction and using the correct nonce.
func (s *PublicBRCXTransactionPoolAPI) SendOrderRawTransaction(ctx context.Context, encodedTx hexutil.Bytes) (common.Hash, error) {
...
// The sender is responsible for signing the transaction and using the correct nonce.
func (s *PublicBRCXTransactionPoolAPI) SendLendingRawTransaction(ctx context.Context, encodedTx hexutil.Bytes) (common.Hash, error) {
...
func (s *PublicBRCXTransactionPoolAPI) GetOrderTxMatchByHash(ctx context.Context, hash common.Hash) ([]*tradingstate.OrderItem, error) {
...
func (s *PublicBRCXTransactionPoolAPI) GetOrderPoolContent(ctx context.Context) interface{} {
...
func (s *PublicBRCXTransactionPoolAPI) GetOrderStats(ctx context.Context) interface{} {
...
// The sender is responsible for signing the transaction and using the correct nonce.
func (s *PublicBRCXTransactionPoolAPI) SendOrder(ctx context.Context, msg OrderMsg) (common.Hash, error) {
...
// The sender is responsible for signing the transaction and using the correct nonce.
func (s *PublicBRCXTransactionPoolAPI) SendLending(ctx context.Context, msg LendingMsg) (common.Hash, error) {
...
func (s *PublicBRCXTransactionPoolAPI) GetOrderCount(ctx context.Context, addr common.Address) (*hexutil.Uint64, error) {
...
func (s *PublicBRCXTransactionPoolAPI) GetBestBid(ctx context.Context, baseToken, quoteToken common.Address) (PriceVolume, error) {
	...
	BRCxService := s.b.BRCxService()
	if BRCxService == nil {
		return result, errors.New("not find BRCX service")
	}
	author, err := s.b.GetEngine().Author(block.Header())
	if err != nil {
		return result, err
	}
	BRCxState, err := BRCxService.GetTradingState(block, author)
	if err != nil {
		return result, err
	}
	result.Price, result.Volume = BRCxState.GetBestBidPrice(tradingstate.GetTradingOrderBookHash(baseToken, quoteToken))
...
func (s *PublicBRCXTransactionPoolAPI) GetBestAsk(ctx context.Context, baseToken, quoteToken common.Address) (PriceVolume, error) {
	result := PriceVolume{}
	block := s.b.CurrentBlock()
	if block == nil {
	...
	BRCxService := s.b.BRCxService()
	if BRCxService == nil {
		return result, errors.New("not found BRCX service")
	}
	author, err := s.b.GetEngine().Author(block.Header())
	if err != nil {
		return result, err
	...
	result.Price, result.Volume = BRCxState.GetBestAskPrice(tradingstate.GetTradingOrderBookHash(baseToken, quoteToken))
	if result.Price.Sign() == 0 {
...
func (s *PublicBRCXTransactionPoolAPI) GetBidTree(ctx context.Context, baseToken, quoteToken common.Address) (map[*big.Int]tradingstate.DumpOrderList, error) {
	block := s.b.CurrentBlock()
	if block == nil {
	...
	BRCxService := s.b.BRCxService()
	if BRCxService == nil {
		return nil, errors.New("not find BRCX service")
	}
	author, err := s.b.GetEngine().Author(block.Header())
	if err != nil {
		return nil, err
	...
	result, err := BRCxState.DumpBidTrie(tradingstate.GetTradingOrderBookHash(baseToken, quoteToken))
	if err != nil {
		return nil, err
	}
	return result, nil
...
func (s *PublicBRCXTransactionPoolAPI) GetPrice(ctx context.Context, baseToken, quoteToken common.Address) (*big.Int, error) {
	block := s.b.CurrentBlock()
	if block == nil {
		return nil, errors.New("not find current block")
	}
	...
	price := BRCxState.GetLastPrice(tradingstate.GetTradingOrderBookHash(baseToken, quoteToken))
...
func (s *PublicBRCXTransactionPoolAPI) GetLastEpochPrice(ctx context.Context, baseToken, quoteToken common.Address) (*big.Int, error) {
	block := s.b.CurrentBlock()
	if block == nil {
		return nil, errors.New("not find current block")
	}
	...
	price := BRCxState.GetMediumPriceBeforeEpoch(tradingstate.GetTradingOrderBookHash(baseToken, quoteToken))
	if price == nil || price.Sign() == 0 {
		return common.Big0, errors.New("not find order book's price")
	}
	return price, nil
...
func (s *PublicBRCXTransactionPoolAPI) GetCurrentEpochPrice(ctx context.Context, baseToken, quoteToken common.Address) (*big.Int, error) {
	block := s.b.CurrentBlock()
	if block == nil {
		return nil, errors.New("not find current block")
	}
	...
	price, _ := BRCxState.GetMediumPriceAndTotalAmount(tradingstate.GetTradingOrderBookHash(baseToken, quoteToken))
	if price == nil || price.Sign() == 0 {
		return common.Big0, errors.New("not find order book's price")
	}
	return price, nil
...
func (s *PublicBRCXTransactionPoolAPI) GetAskTree(ctx context.Context, baseToken, quoteToken common.Address) (map[*big.Int]tradingstate.DumpOrderList, error) {
	block := s.b.CurrentBlock()
	if block == nil {
		return nil, errors.New("not find current block")
	}
	...
	result, err := BRCxState.DumpAskTrie(tradingstate.GetTradingOrderBookHash(baseToken, quoteToken))
	if err != nil {
		return nil, err
	}
	return result, nil
...
func (s *PublicBRCXTransactionPoolAPI) GetOrderById(ctx context.Context, baseToken, quoteToken common.Address, orderId uint64) (interface{}, error) {
	block := s.b.CurrentBlock()
	if block == nil {
		return nil, errors.New("not find current block")
	}
	...
	orderitem := BRCxState.GetOrder(tradingstate.GetTradingOrderBookHash(baseToken, quoteToken), orderIdHash)
...
func (s *PublicBRCXTransactionPoolAPI) GetTradingOrderBookInfo(ctx context.Context, baseToken, quoteToken common.Address) (*tradingstate.DumpOrderBookInfo, error) {
	block := s.b.CurrentBlock()
	if block == nil {
		return nil, errors.New("not find current block")
	}
	...
	result, err := BRCxState.DumpOrderBookInfo(tradingstate.GetTradingOrderBookHash(baseToken, quoteToken))
	if err != nil {
		return nil, err
	}
	return result, nil
...
func (s *PublicBRCXTransactionPoolAPI) GetLiquidationPriceTree(ctx context.Context, baseToken, quoteToken common.Address) (map[*big.Int]tradingstate.DumpLendingBook, error) {
	block := s.b.CurrentBlock()
	if block == nil {
		return nil, errors.New("not find current block")
	}
	...
	result, err := BRCxState.DumpLiquidationPriceTrie(tradingstate.GetTradingOrderBookHash(baseToken, quoteToken))
	if err != nil {
		return nil, err
	}
	return result, nil
...
func (s *PublicBRCXTransactionPoolAPI) GetInvestingTree(ctx context.Context, lendingToken common.Address, term uint64) (map[*big.Int]lendingstate.DumpOrderList, error) {
	block := s.b.CurrentBlock()
	if block == nil {
		return nil, errors.New("not find current block")
	}
	...
	if lendingService == nil {
		return nil, errors.New("BRCX Lending service not found")
	}
	author, err := s.b.GetEngine().Author(block.Header())
	if err != nil {
		return nil, err
...
func (s *PublicBRCXTransactionPoolAPI) GetBorrowingTree(ctx context.Context, lendingToken common.Address, term uint64) (map[*big.Int]lendingstate.DumpOrderList, error) {
	block := s.b.CurrentBlock()
	if block == nil {
		return nil, errors.New("not find current block")
	}
...
func (s *PublicBRCXTransactionPoolAPI) GetLendingOrderBookInfo(tx context.Context, lendingToken common.Address, term uint64) (*lendingstate.DumpOrderBookInfo, error) {
	block := s.b.CurrentBlock()
	if block == nil {
		return nil, errors.New("not find current block")
	}
...
func (s *PublicBRCXTransactionPoolAPI) getLendingOrderTree(ctx context.Context, lendingToken common.Address, term uint64) (map[*big.Int]lendingstate.LendingItem, error) {
	block := s.b.CurrentBlock()
	if block == nil {
		return nil, errors.New("not find current block")
	}
	lendingService := s.b.LendingService()
	if lendingService == nil {
		return nil, errors.New("not find BRCX Lending service")
	}
	author, err := s.b.GetEngine().Author(block.Header())
	if err != nil {
		return nil, err
...
func (s *PublicBRCXTransactionPoolAPI) GetLendingTradeTree(ctx context.Context, lendingToken common.Address, term uint64) (map[*big.Int]lendingstate.LendingTrade, error) {
	block := s.b.CurrentBlock()
	if block == nil {
		return nil, errors.New("not find current block")
	}
...
func (s *PublicBRCXTransactionPoolAPI) GetLiquidationTimeTree(ctx context.Context, lendingToken common.Address, term uint64) (map[*big.Int]lendingstate.DumpOrderList, error) {
	block := s.b.CurrentBlock()
	if block == nil {
		return nil, errors.New("not find current block")
	}
...
func (s *PublicBRCXTransactionPoolAPI) GetLendingOrderCount(ctx context.Context, addr common.Address) (*hexutil.Uint64, error) {
	block := s.b.CurrentBlock()
	if block == nil {
		return nil, errors.New("not find current block")
	}
...
func (s *PublicBRCXTransactionPoolAPI) GetBestInvesting(ctx context.Context, lendingToken common.Address, term uint64) (InterestVolume, error) {
	...
	lendingService := s.b.LendingService()
	if lendingService == nil {
		return result, errors.New("not find BRCX Lending service")
	}
	author, err := s.b.GetEngine().Author(block.Header())
	if err != nil {
		return result, err
...
func (s *PublicBRCXTransactionPoolAPI) GetBestBorrowing(ctx context.Context, lendingToken common.Address, term uint64) (InterestVolume, error) {
	result := InterestVolume{}
	block := s.b.CurrentBlock()
	if block == nil {
		return result, errors.New("not find current block")
...
func (s *PublicBRCXTransactionPoolAPI) GetBids(ctx context.Context, baseToken, quoteToken common.Address) (map[*big.Int]*big.Int, error) {
	block := s.b.CurrentBlock()
	if block == nil {
		return nil, errors.New("not find current block")
	}
	...
	result, err := BRCxState.GetBids(tradingstate.GetTradingOrderBookHash(baseToken, quoteToken))
	if err != nil {
		return nil, err
	}
	return result, nil
...
func (s *PublicBRCXTransactionPoolAPI) GetAsks(ctx context.Context, baseToken, quoteToken common.Address) (map[*big.Int]*big.Int, error) {
	block := s.b.CurrentBlock()
	if block == nil {
		return nil, errors.New("not find current block")
	}
	...
	result, err := BRCxState.GetAsks(tradingstate.GetTradingOrderBookHash(baseToken, quoteToken))
	if err != nil {
		return nil, err
	}
	return result, nil
...
func (s *PublicBRCXTransactionPoolAPI) GetInvests(ctx context.Context, lendingToken common.Address, term uint64) (map[*big.Int]*big.Int, error) {
	block := s.b.CurrentBlock()
	if block == nil {
		return nil, errors.New("not find current block")
	}
...
func (s *PublicBRCXTransactionPoolAPI) GetBorrows(ctx context.Context, lendingToken common.Address, term uint64) (map[*big.Int]*big.Int, error) {
	block := s.b.CurrentBlock()
	if block == nil {
		return nil, errors.New("not find current block")
	}
...
func (s *PublicBRCXTransactionPoolAPI) GetLendingTxMatchByHash(ctx context.Context, hash common.Hash) ([]*lendingstate.LendingItem, error) {
	var tx *types.Transaction
	if tx, _, _, _ = rawdb.ReadTransaction(s.b.ChainDb(), hash); tx == nil {
		if tx = s.b.GetPoolTransaction(hash); tx == nil {
...
// GetLiquidatedTradesByTxHash returns trades which closed by BRCX protocol at the tx of the give hash
func (s *PublicBRCXTransactionPoolAPI) GetLiquidatedTradesByTxHash(ctx context.Context, hash common.Hash) (lendingstate.FinalizedResult, error) {
	var tx *types.Transaction
	if tx, _, _, _ = rawdb.ReadTransaction(s.b.ChainDb(), hash); tx == nil {
		if tx = s.b.GetPoolTransaction(hash); tx == nil {
...
func (s *PublicBRCXTransactionPoolAPI) GetLendingOrderById(ctx context.Context, lendingToken common.Address, term uint64, orderId uint64) (lendingstate.LendingItem, error) {
	...
	lendingService := s.b.LendingService()
	if lendingService == nil {
		return lendingItem, errors.New("not find BRCX lending service")
	}
	author, err := s.b.GetEngine().Author(block.Header())
	if err != nil {
...
func (s *PublicBRCXTransactionPoolAPI) GetLendingTradeById(ctx context.Context, lendingToken common.Address, term uint64, tradeId uint64) (lendingstate.LendingTrade, error) {
...
func GetSignersFromBlocks(b Backend, blockNumber uint64, blockHash common.Hash, masternodes []common.Address) ([]common.Address, error) {
	...
	if engine, ok := b.GetEngine().(*BRDPoS.BRDPoS); ok {
...
func (s *PublicBlockChainAPI) GetStakerROI() float64 {
	...
	lastCheckpointNumber := blockNumber - (blockNumber % s.b.ChainConfig().BRDPoS.Epoch) - s.b.ChainConfig().BRDPoS.Epoch // calculate for 2 epochs ago
	...
	masternodeReward := new(big.Int).Mul(new(big.Int).SetUint64(s.b.ChainConfig().BRDPoS.Reward), new(big.Int).SetUint64(params.Ether))
...
func (s *PublicBlockChainAPI) GetStakerROIMasternode(masternode common.Address) float64 {
	...
	blockNumber := s.b.CurrentBlock().Number().Uint64()
	lastCheckpointNumber := blockNumber - blockNumber%s.b.ChainConfig().BRDPoS.Epoch
	totalCap := new(big.Int).SetUint64(0)
