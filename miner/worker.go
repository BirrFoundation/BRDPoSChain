...
func (w *worker) update() {
	...
	minePeriod := 2
	MinePeriodCh := w.engine.(*BRDPoS.BRDPoS).MinePeriodCh
	defer close(MinePeriodCh)
	NewRoundCh := w.engine.(*BRDPoS.BRDPoS).NewRoundCh
	...
	for {
		...
		case ev := <-w.txsCh:
			...
			if atomic.LoadInt32(&w.mining) == 0 {
				...
				if w.config.BRDPoS != nil && w.config.BRDPoS.Period == 0 {
					w.commitNewWork()
				}
			}
...
func (w *worker) wait() {
	for {
		...
		for result := range w.recv {
			...
			block := result.Block
			if w.config.BRDPoS != nil && block.NumberU64() >= w.config.BRDPoS.Epoch && len(block.Validator()) == 0 {
			...
			if work.config.BRDPoS != nil {
				// epoch block
				isEpochSwitchBlock, _, err := w.engine.(*BRDPoS.BRDPoS).IsEpochSwitch(block.Header())
				if err != nil {
			...
			if w.config.BRDPoS != nil {
				c := w.engine.(*BRDPoS.BRDPoS)
...
func (w *worker) makeCurrent(parent *types.Block, header *types.Header) error {
	...
	var BRCxState *tradingstate.TradingStateDB
	var lendingState *lendingstate.LendingStateDB
	if w.config.BRDPoS != nil {
		BRCX := w.eth.GetBRCX()
		BRCxState, err = BRCX.GetTradingState(parent, author)
		if err != nil {
			log.Error("Failed to get BRCx state ", "number", parent.Number(), "err", err)
			return err
		}
		lending := w.eth.GetBRCXLending()
...
func (w *worker) commitNewWork() {
	...
	if w.config.BRDPoS != nil {
		isEpochSwitchBlock, _, err := w.engine.(*BRDPoS.BRDPoS).IsEpochSwitch(header)
		if err != nil {
	...
	if atomic.LoadInt32(&w.mining) == 1 {
		...
		if w.config.BRDPoS != nil && w.chain.Config().IsTIPBRCXMiner(header.Number) {
			BRCX := w.eth.GetBRCX()
			BRCXLending := w.eth.GetBRCXLending()
			if BRCX != nil && header.Number.Uint64() > w.config.BRDPoS.Epoch {
				isEpochSwitchBlock, epochNumber, err := w.engine.(*BRDPoS.BRDPoS).IsEpochSwitch(header)
				if err != nil {
					log.Error("[commitNewWork] fail to check if block is epoch switch block when performing BRCX and BRCXLending operations", "BlockNum", header.Number, "Hash", header.Hash())
				}

				if isEpochSwitchBlock {
					err := BRCX.UpdateMediumPriceBeforeEpoch(epochNumber, work.tradingState, work.state)
					if err != nil {
					...
					tradingTxMatches, tradingMatchingResults = BRCX.ProcessOrderPending(header, w.coinbase, w.chain, tradingOrderPending, work.state, work.tradingState)
					...
					lendingInput, lendingMatchingResults = BRCXLending.ProcessOrderPending(header, w.coinbase, w.chain, lendingOrderPending, work.state, work.lendingState, work.tradingState)
					...
					if header.Number.Uint64()%w.config.BRDPoS.Epoch == common.LiquidateLendingTradeBlock {
						updatedTrades, liquidatedTrades, autoRepayTrades, autoTopUpTrades, autoRecallTrades, err = BRCXLending.ProcessLiquidationData(header, w.chain, work.state, work.tradingState, work.lendingState)
						if err != nil {
				...
				if len(tradingTxMatches) > 0 {
					...
					tx := types.NewTransaction(nonce, common.BRCXAddrBinary, big.NewInt(0), txMatchGasLimit, big.NewInt(0), txMatchBytes)
				...
				if len(lendingInput) > 0 {
					...
					nonce := work.state.GetNonce(w.coinbase)
					lendingTx := types.NewTransaction(nonce, common.BRCXLendingAddressBinary, big.NewInt(0), txMatchGasLimit, big.NewInt(0), lendingDataBytes)
				...
				if len(updatedTrades) > 0 {
					...
					nonce := work.state.GetNonce(w.coinbase)
					finalizedTx := types.NewTransaction(nonce, common.BRCXLendingFinalizedTradeAddressBinary, big.NewInt(0), txMatchGasLimit, big.NewInt(0), finalizedTradeData)
			...
			BRCxStateRoot := work.tradingState.IntermediateRoot()
			LendingStateRoot := work.lendingState.IntermediateRoot()
			txData := append(BRCxStateRoot.Bytes(), LendingStateRoot.Bytes()...)
...
func (w *Work) commitTransactions(mux *event.TypeMux, balanceFee map[common.Address]*big.Int, txs *types.TransactionsByPriceAndNonce, specialTxs types.Transactions, bc *core.BlockChain, coinbase common.Address, pendingLogsFeed *event.Feed) {
	...
	for _, tx := range specialTxs {
		...
		data := tx.Data()
		// validate minFee slot for BRCZ
		if tx.IsBRCZApplyTransaction() {
			copyState, _ := bc.State()
			if err := core.ValidateBRCZApplyTransaction(bc, nil, copyState, common.BytesToAddress(data[4:])); err != nil {
				log.Debug("BRCZApply: invalid token", "token", common.BytesToAddress(data[4:]).Hex())
				txs.Pop()
				continue
			}
		}
		// validate balance slot, token decimal for BRCX
		if tx.IsBRCXApplyTransaction() {
			copyState, _ := bc.State()
			if err := core.ValidateBRCXApplyTransaction(bc, nil, copyState, common.BytesToAddress(data[4:])); err != nil {
				log.Debug("BRCXApply: invalid token", "token", common.BytesToAddress(data[4:]).Hex())
				txs.Pop()
				continue
			}
		}
		...
		if *to == common.BlockSignersBinary {
			...
			if blkNumber >= w.header.Number.Uint64() || blkNumber <= w.header.Number.Uint64()-w.config.BRDPoS.Epoch*2 {
