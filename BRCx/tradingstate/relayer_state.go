...
func AddTokenBalance(addr common.Address, value *big.Int, token common.Address, statedb *state.StateDB) error {
	...
	if token == common.BRCNativeAddressBinary {
		...
		balance = statedb.GetBalance(addr)
		log.Debug("ApplyBRCXMatchedTransaction settle balance: ADD BRC NATIVE BALANCE AFTER", "token", token.String(), "address", addr.String(), "balance", balance, "orderValue", value)
...
func SubTokenBalance(addr common.Address, value *big.Int, token common.Address, statedb *state.StateDB) error {
	...
	if token == common.BRCNativeAddressBinary {
		...
		balance = statedb.GetBalance(addr)
		log.Debug("ApplyBRCXMatchedTransaction settle balance: SUB BRC NATIVE BALANCE AFTER", "token", token.String(), "address", addr.String(), "balance", balance, "orderValue", value)
		return nil
	}
...
func CheckSubTokenBalance(addr common.Address, value *big.Int, token common.Address, statedb *state.StateDB, mapBalances map[common.Address]map[common.Address]*big.Int) (*big.Int, error) {
	...
	if token == common.BRCNativeAddressBinary {
		...
		log.Debug("CheckSubTokenBalance settle balance: SUB BRC NATIVE BALANCE ", "token", token.String(), "address", addr.String(), "balance", balance, "value", value, "newBalance", newBalance)
...
func CheckAddTokenBalance(addr common.Address, value *big.Int, token common.Address, statedb *state.StateDB, mapBalances map[common.Address]map[common.Address]*big.Int) (*big.Int, error) {
	...
	if token == common.BRCNativeAddressBinary {
		...
		log.Debug("CheckAddTokenBalance settle balance: ADD BRC NATIVE BALANCE ", "token", token.String(), "address", addr.String(), "balance", balance, "value", value, "newBalance", newBalance)
		return newBalance, nil
	}
	// TRC tokens
	if statedb.Exist(token) {
