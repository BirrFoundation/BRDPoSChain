...
func VerifyBalance(statedb *state.StateDB, BRCxStateDb *TradingStateDB, order *types.OrderTransaction, baseDecimal, quoteDecimal *big.Int) error {
	var quotePrice *big.Int
	if order.QuoteToken() != common.BRCNativeAddressBinary {
		quotePrice = BRCxStateDb.GetLastPrice(GetTradingOrderBookHash(order.QuoteToken(), common.BRCNativeAddressBinary))
		log.Debug("TryGet quotePrice QuoteToken/BRC", "quotePrice", quotePrice)
		if quotePrice == nil || quotePrice.Sign() == 0 {
			inversePrice := BRCxStateDb.GetLastPrice(GetTradingOrderBookHash(common.BRCNativeAddressBinary, order.QuoteToken()))
			...
			if inversePrice != nil && inversePrice.Sign() > 0 {
				...
				log.Debug("TryGet quotePrice after get inversePrice BRC/QuoteToken", "quotePrice", quotePrice, "quoteTokenDecimal", quoteDecimal)
			}
		}
	} else {
