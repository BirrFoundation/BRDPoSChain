...
func (BRCx *BRCX) CommitOrder(header *types.Header, coinbase common.Address, chain consensus.ChainContext, statedb *state.StateDB, tradingStateDB *tradingstate.TradingStateDB, orderBook common.Hash, order *tradingstate.OrderItem) ([]map[string]string, []*tradingstate.OrderItem, error) {
	BRCxSnap := tradingStateDB.Snapshot()
	dbSnap := statedb.Snapshot()
	trades, rejects, err := BRCx.ApplyOrder(header, coinbase, chain, statedb, tradingStateDB, orderBook, order)
	if err != nil {
		tradingStateDB.RevertToSnapshot(BRCxSnap)
...
func (BRCx *BRCX) ApplyOrder(header *types.Header, coinbase common.Address, chain consensus.ChainContext, statedb *state.StateDB, tradingStateDB *tradingstate.TradingStateDB, orderBook common.Hash, order *tradingstate.OrderItem) ([]map[string]string, []*tradingstate.OrderItem, error) {
	var (
	...
	if order.Status == tradingstate.OrderStatusCancelled {
		err, reject := BRCx.ProcessCancelOrder(header, tradingStateDB, statedb, chain, coinbase, orderBook, order)
		if err != nil || reject {
	...
	if orderType == tradingstate.Market {
		...
		trades, rejects, err = BRCx.processMarketOrder(coinbase, chain, statedb, tradingStateDB, orderBook, order)
		if err != nil {
		...
		trades, rejects, err = BRCx.processLimitOrder(coinbase, chain, statedb, tradingStateDB, orderBook, order)
		if err != nil {
...
func (BRCx *BRCX) processMarketOrder(coinbase common.Address, chain consensus.ChainContext, statedb *state.StateDB, tradingStateDB *tradingstate.TradingStateDB, orderBook common.Hash, order *tradingstate.OrderItem) ([]map[string]string, []*tradingstate.OrderItem, error) {
	var (
		trades     []map[string]string
	...
	if side == tradingstate.Bid {
		...
		for quantityToTrade.Cmp(zero) > 0 && bestPrice.Cmp(zero) > 0 {
			quantityToTrade, newTrades, newRejects, err = BRCx.processOrderList(coinbase, chain, statedb, tradingStateDB, tradingstate.Ask, orderBook, bestPrice, quantityToTrade, order)
			if err != nil {
				return nil, nil, err
			}
		...
		log.Debug("processMarketOrder ", "side", side, "bestPrice", bestPrice, "quantityToTrade", quantityToTrade, "volume", volume)
		for quantityToTrade.Cmp(zero) > 0 && bestPrice.Cmp(zero) > 0 {
			quantityToTrade, newTrades, newRejects, err = BRCx.processOrderList(coinbase, chain, statedb, tradingStateDB, tradingstate.Bid, orderBook, bestPrice, quantityToTrade, order)
			if err != nil {
				return nil, nil, err
			}
			trades = append(trades, newTrades...)
...
func (BRCx *BRCX) processLimitOrder(coinbase common.Address, chain consensus.ChainContext, statedb *state.StateDB, tradingStateDB *tradingstate.TradingStateDB, orderBook common.Hash, order *tradingstate.OrderItem) ([]map[string]string, []*tradingstate.OrderItem, error) {
	var (
		trades     []map[string]string
		newTrades  []map[string]string
		rejects    []*tradingstate.OrderItem
	...
	if side == tradingstate.Bid {
		...
		for quantityToTrade.Cmp(zero) > 0 && price.Cmp(minPrice) >= 0 && minPrice.Cmp(zero) > 0 {
			...
			quantityToTrade, newTrades, newRejects, err = BRCx.processOrderList(coinbase, chain, statedb, tradingStateDB, tradingstate.Ask, orderBook, minPrice, quantityToTrade, order)
			if err != nil {
				return nil, nil, err
			}
			trades = append(trades, newTrades...)
		...
		for quantityToTrade.Cmp(zero) > 0 && price.Cmp(maxPrice) <= 0 && maxPrice.Cmp(zero) > 0 {
			...
			quantityToTrade, newTrades, newRejects, err = BRCx.processOrderList(coinbase, chain, statedb, tradingStateDB, tradingstate.Bid, orderBook, maxPrice, quantityToTrade, order)
			if err != nil {
				return nil, nil, err
			}
			trades = append(trades, newTrades...)
...
func (BRCx *BRCX) processOrderList(coinbase common.Address, chain consensus.ChainContext, statedb *state.StateDB, tradingStateDB *tradingstate.TradingStateDB, side string, orderBook common.Hash, price *big.Int, quantityStillToTrade *big.Int, order *tradingstate.OrderItem) (*big.Int, []map[string]string, []*tradingstate.OrderItem, error) {
	...
	for quantityToTrade.Sign() > 0 {
		...
		if oldestOrder.QuoteToken != common.BRCNativeAddressBinary {
			quotePrice = tradingStateDB.GetLastPrice(tradingstate.GetTradingOrderBookHash(oldestOrder.QuoteToken, common.BRCNativeAddressBinary))
			log.Debug("TryGet quotePrice QuoteToken/BRC", "quotePrice", quotePrice)
			if quotePrice == nil || quotePrice.Sign() == 0 {
				inversePrice := tradingStateDB.GetLastPrice(tradingstate.GetTradingOrderBookHash(common.BRCNativeAddressBinary, oldestOrder.QuoteToken))
				quoteTokenDecimal, err := BRCx.GetTokenDecimal(chain, statedb, oldestOrder.QuoteToken)
				if err != nil || quoteTokenDecimal.Sign() == 0 {
					return nil, nil, nil, fmt.Errorf("fail to get tokenDecimal: Token: %v . Err: %v", oldestOrder.QuoteToken.String(), err)
				}
				log.Debug("TryGet inversePrice BRC/QuoteToken", "inversePrice", inversePrice)
				if inversePrice != nil && inversePrice.Sign() > 0 {
					quotePrice = new(big.Int).Mul(common.BasePrice, quoteTokenDecimal)
					quotePrice = new(big.Int).Div(quotePrice, inversePrice)
					log.Debug("TryGet quotePrice after get inversePrice BRC/QuoteToken", "quotePrice", quotePrice, "quoteTokenDecimal", quoteTokenDecimal)
				}
			}
		} else {
			quotePrice = common.BasePrice
		}
		tradedQuantity, rejectMaker, settleBalanceResult, err := BRCx.getTradeQuantity(quotePrice, coinbase, chain, statedb, order, &oldestOrder, maxTradedQuantity)
...
func (BRCx *BRCX) getTradeQuantity(quotePrice *big.Int, coinbase common.Address, chain consensus.ChainContext, statedb *state.StateDB, takerOrder *tradingstate.OrderItem, makerOrder *tradingstate.OrderItem, quantityToTrade *big.Int) (*big.Int, bool, *tradingstate.SettleBalance, error) {
	baseTokenDecimal, err := BRCx.GetTokenDecimal(chain, statedb, makerOrder.BaseToken)
	...
	quoteTokenDecimal, err := BRCx.GetTokenDecimal(chain, statedb, makerOrder.QuoteToken)
	if err != nil || quoteTokenDecimal.Sign() == 0 {
		return tradingstate.Zero, false, nil, fmt.Errorf("fail to get tokenDecimal: Token: %v . Err: %v", makerOrder.QuoteToken.String(), err)
	}
	if makerOrder.QuoteToken == common.BRCNativeAddressBinary {
...
func GetTradeQuantity(takerSide string, takerFeeRate *big.Int, takerBalance *big.Int, makerPrice *big.Int, makerFeeRate *big.Int, makerBalance *big.Int, baseTokenDecimal *big.Int, quantityToTrade *big.Int) (*big.Int, bool) {
	if takerSide == tradingstate.Bid {
		...
		takerFee = big.NewInt(0).Div(takerFee, common.BRCXBaseFee)
		...
		if takerBalance.Cmp(takerOutTotal) >= 0 && makerBalance.Cmp(makerOutTotal) >= 0 {
			...
			newQuantityTrade = new(big.Int).Mul(newQuantityTrade, common.BRCXBaseFee)
			newQuantityTrade = new(big.Int).Div(newQuantityTrade, new(big.Int).Add(common.BRCXBaseFee, takerFeeRate))
		...
		makerFee = new(big.Int).Div(makerFee, common.BRCXBaseFee)
		...
		if takerBalance.Cmp(takerOutTotal) >= 0 && makerBalance.Cmp(makerOutTotal) >= 0 {
			...
			newQuantityTrade = new(big.Int).Mul(newQuantityTrade, common.BRCXBaseFee)
			newQuantityTrade = new(big.Int).Div(newQuantityTrade, new(big.Int).Add(common.BRCXBaseFee, makerFeeRate))
			newQuantityTrade = new(big.Int).Div(newQuantityTrade, makerPrice)
			log.Debug("Reject order maker , not enough balance ", "makerBalance", makerBalance, " makerOutTotal", makerOutTotal)
...
func DoSettleBalance(coinbase common.Address, takerOrder, makerOrder *tradingstate.OrderItem, settleBalance *tradingstate.SettleBalance, statedb *state.StateDB) error {
	...
	//log.Debug("ApplyBRCXMatchedTransaction settle fee for relayers",
	//	"takerRelayerOwner", takerExOwner,
	//	"takerFeeToken", quoteToken, "takerFee", settleBalanceResult[takerAddr][BRCx.Fee].(*big.Int),
	//	"makerRelayerOwner", makerExOwner,
	//	"makerFeeToken", quoteToken, "makerFee", settleBalanceResult[makerAddr][BRCx.Fee].(*big.Int))
	// takerFee
...
func (BRCx *BRCX) ProcessCancelOrder(header *types.Header, tradingStateDB *tradingstate.TradingStateDB, statedb *state.StateDB, chain consensus.ChainContext, coinbase common.Address, orderBook common.Hash, order *tradingstate.OrderItem) (error, bool) {
	...
	baseTokenDecimal, err := BRCx.GetTokenDecimal(chain, statedb, order.BaseToken)
	if err != nil || baseTokenDecimal.Sign() == 0 {
		log.Debug("Fail to get tokenDecimal ", "Token", order.BaseToken.String(), "err", err)
		return err, false
	}
	// order: basic order information (includes orderId, orderHash, baseToken, quoteToken) which user send to BRCx to cancel order
	...
	var tokenCancelFee, tokenPriceInBRC *big.Int
	if !chain.Config().IsTIPBRCXCancellationFee(header.Number) {
		tokenCancelFee = getCancelFeeV1(baseTokenDecimal, feeRate, &originOrder)
		tokenPriceInBRC = common.Big0
	} else {
		tokenCancelFee, tokenPriceInBRC = BRCx.getCancelFee(chain, statedb, tradingStateDB, &originOrder, feeRate)
	}
	...
	// relayers pay BRC for masternode
	...
	extraData, _ := json.Marshal(struct {
		CancelFee       string
		TokenPriceInBRC string
	}{
		CancelFee:       tokenCancelFee.Text(10),
		TokenPriceInBRC: tokenPriceInBRC.Text(10),
	})
...
// deprecated after hardfork at TIPBRCXCancellationFee
func getCancelFeeV1(baseTokenDecimal *big.Int, feeRate *big.Int, order *tradingstate.OrderItem) *big.Int {
	...
	if order.Side == tradingstate.Ask {
		...
		cancelFee = new(big.Int).Div(cancelFee, common.BRCXBaseCancelFee)
	} else {
		// BUY 1 BTC => BRC with Price : 10000
...
// return tokenQuantity, tokenPriceInBRC
func (BRCx *BRCX) getCancelFee(chain consensus.ChainContext, statedb *state.StateDB, tradingStateDb *tradingstate.TradingStateDB, order *tradingstate.OrderItem, feeRate *big.Int) (*big.Int, *big.Int) {
	if feeRate == nil || feeRate.Sign() == 0 {
		return common.Big0, common.Big0
	}
	var cancelFee, tokenPriceInBRC *big.Int
	var err error
	if order.Side == tradingstate.Ask {
		cancelFee, tokenPriceInBRC, err = BRCx.ConvertBRCToToken(chain, statedb, tradingStateDb, order.BaseToken, common.RelayerCancelFee)
	} else {
		cancelFee, tokenPriceInBRC, err = BRCx.ConvertBRCToToken(chain, statedb, tradingStateDb, order.QuoteToken, common.RelayerCancelFee)
	}
	if err != nil {
		return common.Big0, common.Big0
	}
	return cancelFee, tokenPriceInBRC
}

func (BRCx *BRCX) UpdateMediumPriceBeforeEpoch(epochNumber uint64, tradingStateDB *tradingstate.TradingStateDB, statedb *state.StateDB) error {
	...
	if BRCx.IsSDKNode() {
		if err := BRCx.LogEpochPrice(epochNumber, epochPriceResult); err != nil {
...
// orderbook hash genereted from baseToken, quoteToken at BRDPoSChain/BRCx/tradingstate/common.go:214
func (BRCx *BRCX) LogEpochPrice(epochNumber uint64, epochPriceResult map[common.Hash]*big.Int) error {
	db := BRCx.GetMongoDB()
	db.InitBulk()
