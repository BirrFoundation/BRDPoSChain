...
func Test_getCancelFee(t *testing.T) {
	...
	BRCx.SetTokenDecimal(testTokenB, tokenBDecimal)
	...
	tradingStateDb.SetMediumPriceBeforeEpoch(tradingstate.GetTradingOrderBookHash(common.BRCNativeAddressBinary, testTokenB), tokenBDecimal)
	...
	tests := []struct {
		...
		// test getCancelFee: SELL
		{
			"TokenA/BRC test getCancelFee:: SELL",
			CancelFeeArg{
				feeRate: new(big.Int).SetUint64(10), // 10/10000= 0.1%
				order: &tradingstate.OrderItem{
		...
		// test getCancelFee:: BUY
		{
			"TokenA/BRC test getCancelFee:: BUY",
			CancelFeeArg{
				feeRate: new(big.Int).SetUint64(10), // 10/10000= 0.1%
				order: &tradingstate.OrderItem{
					Quantity:   new(big.Int).SetUint64(10000),
		...
		// test getCancelFee: SELL
		{
			"BRC/TokenA test getCancelFee:: SELL",
			CancelFeeArg{
				feeRate: new(big.Int).SetUint64(10), // 10/10000= 0.1%
				order: &tradingstate.OrderItem{
					BaseToken:  common.BRCNativeAddressBinary,
		...
		// test getCancelFee:: BUY
		{
			"BRC/TokenA test getCancelFee:: BUY",
			CancelFeeArg{
				feeRate: new(big.Int).SetUint64(10), // 10/10000= 0.1%
				order: &tradingstate.OrderItem{
					Quantity:   new(big.Int).SetUint64(10000),
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got, _ := BRCx.getCancelFee(nil, nil, tradingStateDb, tt.args.order, tt.args.feeRate); !reflect.DeepEqual(got, tt.want) {
	...
	if fee, _ := BRCx.getCancelFee(nil, nil, tradingStateDb, tokenCOrder.order, tokenCOrder.feeRate); fee != nil && fee.Sign() != 0 {
	...
	BRCx.SetTokenDecimal(testTokenD, big.NewInt(0))
	...
	if fee, _ := BRCx.getCancelFee(nil, nil, tradingStateDb, tokenDOrder.order, tokenDOrder.feeRate); fee != nil && fee.Sign() != 0 {
		t.Errorf("getCancelFee() = %v, want %v", fee, common.Big0)
	}
