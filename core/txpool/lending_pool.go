...
// blockChain_BRCx add order state
...
func (pool *LendingPool) reset(oldHead, newblock *types.Block) {
	if !pool.chainconfig.IsTIPBRCXReceiver(pool.chain.CurrentBlock().Number()) || pool.chain.Config().BRDPoS == nil || pool.chain.CurrentBlock().NumberU64() <= pool.chain.Config().BRDPoS.Epoch {
		return
	}
...
func (pool *LendingPool) validateBalance(cloneStateDb *state.StateDB, cloneLendingStateDb *lendingstate.LendingStateDB, tx *types.LendingTransaction, collateralToken common.Address) error {
	BRDPoSEngine, ok := pool.chain.Engine().(*BRDPoS.BRDPoS)
	if !ok {
		return core.ErrNotBRDPoS
	}
	BRCXServ := BRDPoSEngine.GetBRCXService()
	lendingServ := BRDPoSEngine.GetLendingService()
	if BRCXServ == nil {
		return errors.New("BRCx not found in order validation")
	}
	lendingTokenDecimal, err := BRCXServ.GetTokenDecimal(pool.chain, cloneStateDb, tx.LendingToken())
	if err != nil {
	...
	tradingStateDb, err := BRCXServ.GetTradingState(pool.chain.CurrentBlock(), author)
	if err != nil {
	...
	// lendTokenBRCPrice: price of lendingToken in BRC quote
	var lendTokenBRCPrice, collateralPrice, collateralTokenDecimal *big.Int
	if !collateralToken.IsZero() {
		collateralTokenDecimal, err = BRCXServ.GetTokenDecimal(pool.chain, cloneStateDb, collateralToken)
		if err != nil {
			return fmt.Errorf("validateOrder: failed to get collateralTokenDecimal. err: %v", err)
		}
		lendTokenBRCPrice, collateralPrice, err = lendingServ.GetCollateralPrices(pool.chain.CurrentHeader(), pool.chain, cloneStateDb, cloneTradingStateDb, collateralToken, tx.LendingToken())
		if err != nil {
			return err
		}
		if lendTokenBRCPrice == nil || lendTokenBRCPrice.Sign() <= 0 || collateralPrice == nil || collateralPrice.Sign() <= 0 {
			log.Debug("ValidateLending: ErrInvalidCollateralPrice", "lendTokenBRCPrice", lendTokenBRCPrice, "collateralPrice", collateralPrice)
			return lendingstate.ErrInvalidCollateralPrice
		}
	}
	if lendTokenBRCPrice == nil || lendTokenBRCPrice.Sign() == 0 {
		if tx.LendingToken() == common.BRCNativeAddressBinary {
			lendTokenBRCPrice = common.BasePrice
		} else {
			lendTokenBRCPrice, err = lendingServ.GetMediumTradePriceBeforeEpoch(pool.chain, cloneStateDb, cloneTradingStateDb, tx.LendingToken(), common.BRCNativeAddressBinary)
			if err != nil {
				return err
			}
		}
	}
	isBRCXLendingFork := pool.chain.Config().IsTIPBRCXLending(pool.chain.CurrentHeader().Number)
	if err := lendingstate.VerifyBalance(isBRCXLendingFork,
		cloneStateDb,
		cloneLendingStateDb,
		tx.Type(),
		tx.Side(),
...
func (pool *LendingPool) addTx(tx *types.LendingTransaction, local bool) error {
	if !pool.chainconfig.IsTIPBRCXReceiver(pool.chain.CurrentBlock().Number()) {
		return nil
	}
	tx.CacheHash()
