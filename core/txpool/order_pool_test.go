...
func getNonce(t *testing.T, userAddress common.Address) (uint64, error) {
	...
	err = rpcClient.Call(&result, "BRCx_getOrderCount", userAddress)
	if err != nil {
		return 0, err
	}
	s := result.(string)
...
func testSendOrder(t *testing.T, amount, price *big.Int, side string, status string, orderID uint64) {
	...
	msg := &OrderMsg{
		...
		BaseToken:       common.BRCNativeAddressBinary,
...
func testSendOrderBRCUSD(t *testing.T, amount, price *big.Int, side string, status string, orderID uint64) {
...
func testSendOrderBRCBTC(t *testing.T, amount, price *big.Int, side string, status string, orderID uint64) {
...
func TestFilled(t *testing.T) {
	...
	//testSendOrderBRCUSD(t, new(big.Int).Mul(big.NewInt(1000000000000000000), big.NewInt(5000)), BTCUSDPrice, "BUY", "NEW", 0)
	//ETH/BTC
	...
	BRCBTCPrice := new(big.Int).Mul(big.NewInt(10000000000000), big.NewInt(6)) // 0.00006
	time.Sleep(2 * time.Second)
	testSendOrderBRCBTC(t, new(big.Int).Mul(big.NewInt(600000), _1E18), BRCBTCPrice, "BUY", "NEW", 0)
	time.Sleep(2 * time.Second)
	testSendOrderBRCBTC(t, new(big.Int).Mul(big.NewInt(600000), _1E18), BRCBTCPrice, "BUY", "NEW", 0)
	time.Sleep(2 * time.Second)
	testSendOrderBRCBTC(t, new(big.Int).Mul(big.NewInt(1200000), _1E18), BRCBTCPrice, "SELL", "NEW", 0)

	BRCUSDPrice := new(big.Int).Mul(_1E7, big.NewInt(6)) // 0.6
	time.Sleep(2 * time.Second)
	testSendOrderBRCUSD(t, new(big.Int).Mul(big.NewInt(600000), _1E18), BRCUSDPrice, "BUY", "NEW", 0)
	time.Sleep(2 * time.Second)
	testSendOrderBRCUSD(t, new(big.Int).Mul(big.NewInt(600000), _1E18), BRCUSDPrice, "BUY", "NEW", 0)
	time.Sleep(2 * time.Second)
	testSendOrderBRCUSD(t, new(big.Int).Mul(big.NewInt(1200000), _1E18), BRCUSDPrice, "SELL", "NEW", 0)

}

func TestX10Filled(t *testing.T) {
	BRCUSDPrice := new(big.Int).Mul(_1E7, big.NewInt(60)) // 6
	time.Sleep(2 * time.Second)
	testSendOrderBRCUSD(t, new(big.Int).Mul(big.NewInt(600000), _1E18), BRCUSDPrice, "BUY", "NEW", 0)
	time.Sleep(2 * time.Second)
	testSendOrderBRCUSD(t, new(big.Int).Mul(big.NewInt(600000), _1E18), BRCUSDPrice, "BUY", "NEW", 0)
...
func TestCancelOrder(t *testing.T) {
	BRCBTCPrice := new(big.Int).Mul(big.NewInt(10000000000000), big.NewInt(6)) // 0.00006
	testSendOrder(t, new(big.Int).Mul(big.NewInt(600000), _1E18), BRCBTCPrice, "BUY", "NEW", 0)
	time.Sleep(5 * time.Second)
	testSendOrder(t, new(big.Int).Mul(big.NewInt(600000), _1E18), BRCBTCPrice, "BUY", "CANCELLED", 3)
	time.Sleep(5 * time.Second)
