...
func (pool *TxPool) validateTx(tx *types.Transaction, local bool) error {
	...
	*/
	...
	if tx.IsBRCZApplyTransaction() {
		...
		return core.ValidateBRCZApplyTransaction(pool.chain, nil, copyState, common.BytesToAddress(tx.Data()[4:]))
	}

	// validate balance slot, token decimal for BRCX
	if tx.IsBRCXApplyTransaction() {
		copyState := pool.currentState.Copy()
		return core.ValidateBRCXApplyTransaction(pool.chain, nil, copyState, common.BytesToAddress(tx.Data()[4:]))
	}
	return nil
}
