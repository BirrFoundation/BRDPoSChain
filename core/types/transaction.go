...
var (
	...
	skipNonceDestinationAddress = map[common.Address]bool{
		common.BRCXAddrBinary:                         true,
		common.TradingStateAddrBinary:                 true,
		common.BRCXLendingAddressBinary:               true,
		common.BRCXLendingFinalizedTradeAddressBinary: true,
	}
)
...
func (tx *Transaction) IsTradingTransaction() bool {
	to := tx.To()
	return to != nil && *to == common.BRCXAddrBinary
}

func (tx *Transaction) IsLendingTransaction() bool {
	to := tx.To()
	return to != nil && *to == common.BRCXLendingAddressBinary
}

func (tx *Transaction) IsLendingFinalizedTradeTransaction() bool {
	to := tx.To()
	return to != nil && *to == common.BRCXLendingFinalizedTradeAddressBinary
}
...
func (tx *Transaction) IsBRCXApplyTransaction() bool {
	to := tx.To()
	if to == nil {
		return false
	}

	addr := common.BRCXListingSMC
	if common.IsTestnet {
		addr = common.BRCXListingSMCTestNet
	}
	if *to != addr {
		return false
	}
	...
	method := hexutil.Encode(data[0:4])
	return method == common.BRCXApplyMethod
}

func (tx *Transaction) IsBRCZApplyTransaction() bool {
	to := tx.To()
	if to == nil {
		return false
	}
	...
	method := hexutil.Encode(data[0:4])
	return method == common.BRCZApplyMethod
}
