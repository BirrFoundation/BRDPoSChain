...
func (p *StateProcessor) Process(block *types.Block, statedb *state.StateDB, tradingState *tradingstate.TradingStateDB, cfg vm.Config, balanceFee map[common.Address]*big.Int) (types.Receipts, []*types.Log, uint64, error) {
	...
	for i, tx := range block.Transactions() {
		...
		if tx.IsBRCZApplyTransaction() {
			...
			if err := ValidateBRCZApplyTransaction(p.bc, block.Number(), copyState, common.BytesToAddress(tx.Data()[4:])); err != nil {
		...
		if tx.IsBRCXApplyTransaction() {
			copyState := statedb.Copy()
			if err := ValidateBRCXApplyTransaction(p.bc, block.Number(), copyState, common.BytesToAddress(tx.Data()[4:])); err != nil {
				return nil, nil, 0, err
			}
		}
...
func applyTransaction(config *params.ChainConfig, tokensFee map[common.Address]*big.Int, gp *GasPool, statedb *state.StateDB, coinbaseOwner common.Address, blockNumber, baseFee *big.Int, blockHash common.Hash, tx *types.Transaction, usedGas *uint64, evm *vm.EVM) (*types.Receipt, uint64, error, bool) {
	...
	if to != nil {
		...
		if *to == common.TradingStateAddrBinary && config.IsTIPBRCXReceiver(blockNumber) {
			return ApplyEmptyTransaction(config, statedb, blockNumber, blockHash, tx, usedGas)
		}
		if *to == common.BRCXLendingAddressBinary && config.IsTIPBRCXReceiver(blockNumber) {
			return ApplyEmptyTransaction(config, statedb, blockNumber, blockHash, tx, usedGas)
		}
	}
	if tx.IsTradingTransaction() && config.IsTIPBRCXReceiver(blockNumber) {
		return ApplyEmptyTransaction(config, statedb, blockNumber, blockHash, tx, usedGas)
	}
	if tx.IsLendingFinalizedTradeTransaction() && config.IsTIPBRCXReceiver(blockNumber) {
		return ApplyEmptyTransaction(config, statedb, blockNumber, blockHash, tx, usedGas)
	}
	...
	if blockNumber.Cmp(maxBlockNumber) <= 0 {
		...
		if addr, ok := blockMap[currentBlockNumber]; ok {
			if strings.EqualFold(addr, addrFrom) { // case insensitive
				...
				log.Info("address", addr, "with_balance", bal, "BRC")
...
func ApplyTransaction(config *params.ChainConfig, tokensFee map[common.Address]*big.Int, bc *BlockChain, author *common.Address, gp *GasPool, statedb *state.StateDB, BRCxState *tradingstate.TradingStateDB, header *types.Header, tx *types.Transaction, usedGas *uint64, cfg vm.Config) (*types.Receipt, uint64, error, bool) {
	// Create a new context to be used in the EVM environment
	blockContext := NewEVMBlockContext(header, bc, author)
	vmenv := vm.NewEVM(blockContext, vm.TxContext{}, statedb, BRCxState, config, cfg)
