...
func NewBlockChainEx(db ethdb.Database, BRCxDb ethdb.BRCxDatabase, cacheConfig *CacheConfig, chainConfig *params.ChainConfig, engine consensus.Engine, vmConfig vm.Config) (*BlockChain, error) {
	blockchain, err := NewBlockChain(db, cacheConfig, chainConfig, engine, vmConfig)
	if err != nil {
		return nil, err
	}
	if blockchain != nil {
		blockchain.addBRCxDb(BRCxDb)
	}
...
func (bc *BlockChain) addBRCxDb(BRCxDb ethdb.BRCxDatabase) {
	bc.BRCxDb = BRCxDb
}
...
func (bc *BlockChain) loadLastState() error {
	...
	if err != nil {
		...
		engine, ok := bc.Engine().(*BRDPoS.BRDPoS)
		if ok {
			tradingService := engine.GetBRCXService()
			lendingService := engine.GetLendingService()
			if bc.Config().IsTIPBRCX(currentBlock.Number()) && bc.chainConfig.BRDPoS != nil && currentBlock.NumberU64() > bc.chainConfig.BRDPoS.Epoch && tradingService != nil && lendingService != nil {
	...
	if engine, ok := bc.Engine().(*BRDPoS.BRDPoS); ok {
...
func (bc *BlockChain) OrderStateAt(block *types.Block) (*tradingstate.TradingStateDB, error) {
	engine, ok := bc.Engine().(*BRDPoS.BRDPoS)
	if ok {
		BRCXService := engine.GetBRCXService()
		if bc.Config().IsTIPBRCX(block.Number()) && bc.chainConfig.BRDPoS != nil && block.NumberU64() > bc.chainConfig.BRDPoS.Epoch && BRCXService != nil {
			...
			if err == nil {
				return BRCxState, nil
			} else {
				return nil, err
			}
		} else {
			BRCxState, err := BRCXService.GetEmptyTradingState()
			if err == nil {
				return BRCxState, nil
			} else {
				return nil, err
	...
	return nil, errors.New("Get BRCx state fail")
...
func (bc *BlockChain) LendingStateAt(block *types.Block) (*lendingstate.LendingStateDB, error) {
	...
	if ok {
		lendingService := engine.GetLendingService()
		if bc.Config().IsTIPBRCX(block.Number()) && bc.chainConfig.BRDPoS != nil && block.NumberU64() > bc.chainConfig.BRDPoS.Epoch && lendingService != nil {
			author, _ := bc.Engine().Author(block.Header())
		...
		if (common.Rewound == uint64(0)) || ((*head).Number().Uint64() < common.Rewound) {
			if _, err := state.New((*head).Root(), bc.stateCache); err == nil {
				...
				if ok {
					tradingService := engine.GetBRCXService()
					lendingService := engine.GetLendingService()
					if bc.Config().IsTIPBRCXReceiver((*head).Number()) && bc.chainConfig.BRDPoS != nil && (*head).NumberU64() > bc.chainConfig.BRDPoS.Epoch && tradingService != nil && lendingService != nil {
...
func (bc *BlockChain) writeHeadBlock(block *types.Block, writeBlock bool) {
	...
	if bc.chainConfig.BRDPoS != nil && !bc.chainConfig.IsTIPSigning(block.Number()) {
		engine, ok := bc.Engine().(*BRDPoS.BRDPoS)
		if ok {
...
func (bc *BlockChain) HasFullState(block *types.Block) bool {
	...
	engine, _ := bc.Engine().(*BRDPoS.BRDPoS)
	if bc.Config().IsTIPBRCX(block.Number()) && bc.chainConfig.BRDPoS != nil && engine != nil && block.NumberU64() > bc.chainConfig.BRDPoS.Epoch {
		tradingService := engine.GetBRCXService()
		lendingService := engine.GetLendingService()
		author, _ := bc.Engine().Author(block.Header())
...
func (bc *BlockChain) saveData() {
	...
	if !bc.cacheConfig.Disabled {
		...
		if bc.Config().IsTIPBRCX(bc.CurrentBlock().Number()) && bc.chainConfig.BRDPoS != nil && bc.CurrentBlock().NumberU64() > bc.chainConfig.BRDPoS.Epoch && engine != nil {
			tradingService = engine.GetBRCXService()
		...
		for _, offset := range []uint64{0, 1, triesInMemory - 1} {
			if number := bc.CurrentBlock().NumberU64(); number > offset {
				...
				if bc.Config().IsTIPBRCXReceiver(recent.Number()) && bc.chainConfig.BRDPoS != nil && recent.NumberU64() > bc.chainConfig.BRDPoS.Epoch && engine != nil {
...
func (bc *BlockChain) writeBlockWithState(block *types.Block, receipts []*types.Receipt, state *state.StateDB, tradingState *tradingstate.TradingStateDB, lendingState *lendingstate.LendingStateDB) (status WriteStatus, err error) {
	...
	var lendingTrieDb *trie.Database
	var lendingService utils.LendingService
	if bc.Config().IsTIPBRCXReceiver(block.Number()) && bc.chainConfig.BRDPoS != nil && block.NumberU64() > bc.chainConfig.BRDPoS.Epoch && engine != nil {
		tradingService = engine.GetBRCXService()
		if tradingService != nil {
			tradingTrieDb = tradingService.GetStateCache().TrieDB()
		}
	...
	if status == CanonStatTy {
		...
		if bc.chainConfig.BRDPoS != nil && ((block.NumberU64() % bc.chainConfig.BRDPoS.Epoch) == (bc.chainConfig.BRDPoS.Epoch - bc.chainConfig.BRDPoS.Gap)) {
...
func (bc *BlockChain) insertChain(chain types.Blocks, verifySeals bool) (int, []interface{}, []*types.Log, error) {
	...
	for i, block := range chain {
		...
		isSDKNode := false
		engine, _ := bc.Engine().(*BRDPoS.BRDPoS)
		if bc.Config().IsTIPBRCXReceiver(block.Number()) && bc.chainConfig.BRDPoS != nil && engine != nil && block.NumberU64() > bc.chainConfig.BRDPoS.Epoch {
			...
			if tradingService != nil && lendingService != nil {
				...
				if isEpochSwithBlock {
					...
					if block.Number().Uint64()%bc.chainConfig.BRDPoS.Epoch == common.LiquidateLendingTradeBlock {
				...
				if tradingState != nil {
					...
					if gotRoot != expectRoot {
						err = fmt.Errorf("invalid BRCx trading state merke trie got : %s , expect : %s ,parent : %s", gotRoot.Hex(), expectRoot.Hex(), parentRoot.Hex())
						bc.reportBlock(block, nil, err)
						return i, events, coalescedLogs, err
					}
					log.Debug("BRCX Trading State Root", "number", block.NumberU64(), "parent", parentRoot.Hex(), "nextRoot", expectRoot.Hex())
				}
				if lendingState != nil && tradingState != nil {
					...
					log.Debug("BRCX Lending State Root", "number", block.NumberU64(), "parent", parentRoot.Hex(), "nextRoot", expectRoot.Hex())
				}
			}
		}
		...
		case CanonStatTy:
			...
			if bc.chainConfig.IsTIPBRCX(block.Number()) && bc.chainConfig.BRDPoS != nil && block.NumberU64() > bc.chainConfig.BRDPoS.Epoch {
...
func (bc *BlockChain) insertBlock(block *types.Block) ([]interface{}, []*types.Log, error) {
	...
	case CanonStatTy:
		...
		bc.UpdateBlocksHashCache(block)
		if bc.chainConfig.IsTIPBRCXReceiver(block.Number()) && bc.chainConfig.BRDPoS != nil && block.NumberU64() > bc.chainConfig.BRDPoS.Epoch {
			bc.logExchangeData(block)
			bc.logLendingData(block)
		}
	case SideStatTy:
	...
	if bc.chainConfig.BRDPoS != nil {
		// epoch block
		isEpochSwithBlock, _, err := bc.Engine().(*BRDPoS.BRDPoS).IsEpochSwitch(block.Header())
		if err != nil {
...
func (bc *BlockChain) reorg(oldHead, newHead *types.Header) error {
	...
	// Ensure BRDPoS engine committed block will be not reverted
	if BRDPoS, ok := bc.Engine().(*BRDPoS.BRDPoS); ok {
		latestCommittedBlock := BRDPoS.EngineV2.GetLatestCommittedBlockInfo()
...
func (bc *BlockChain) logExchangeData(block *types.Block) {
	...
	BRCXService := engine.GetBRCXService()
	if BRCXService == nil || !BRCXService.IsSDKNode() {
		return
	}
	txMatchBatchData, err := ExtractTradingTransactions(block.Transactions())
	if err != nil {
	...
	for _, txMatchBatch := range txMatchBatchData {
		...
		for _, txMatch := range txMatchBatch.Data {
			...
			if err := BRCXService.SyncDataToSDKNode(takerOrderInTx, txMatchBatch.TxHash, txMatchTime, currentState, trades, rejectedOrders, &dirtyOrderCount); err != nil {
