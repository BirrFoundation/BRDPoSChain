...
func (v *BlockValidator) ValidateTradingOrder(statedb *state.StateDB, BRCxStatedb *tradingstate.TradingStateDB, txMatchBatch tradingstate.TxMatchBatch, coinbase common.Address, header *types.Header) error {
	BRDPoSEngine, ok := v.bc.Engine().(*BRDPoS.BRDPoS)
	if BRDPoSEngine == nil || !ok {
		return ErrNotBRDPoS
	}
	BRCXService := BRDPoSEngine.GetBRCXService()
	if BRCXService == nil {
		return errors.New("BRCx not found")
	}
	...
	for _, txMatch := range txMatchBatch.Data {
		...
		newTrades, newRejectedOrders, err := BRCXService.ApplyOrder(header, coinbase, v.bc, statedb, BRCxStatedb, tradingstate.GetTradingOrderBookHash(order.BaseToken, order.QuoteToken), order)
		if err != nil {
			return err
		}
	...
	if BRCXService.IsSDKNode() {
		v.bc.AddMatchingResult(txMatchBatch.TxHash, tradingResult)
	}
	return nil
}

func (v *BlockValidator) ValidateLendingOrder(statedb *state.StateDB, lendingStateDb *lendingstate.LendingStateDB, BRCxStatedb *tradingstate.TradingStateDB, batch lendingstate.TxLendingBatch, coinbase common.Address, header *types.Header) error {
	BRDPoSEngine, ok := v.bc.Engine().(*BRDPoS.BRDPoS)
	if BRDPoSEngine == nil || !ok {
		return ErrNotBRDPoS
	}
	...
	lendingService := BRDPoSEngine.GetLendingService()
	...
	for _, l := range batch.Data {
		...
		// process Matching Engine
		newTrades, newRejectedOrders, err := lendingService.ApplyOrder(header, coinbase, v.bc, statedb, lendingStateDb, BRCxStatedb, lendingstate.GetLendingOrderBookHash(l.LendingToken, l.Term), l)
		if err != nil {
			return err
		}
