...
func (s *Service) loop() {
	...
	if s.eth != nil {
		...
		txpool = s.eth.TxPool()
		engine = s.eth.Engine().(*BRDPoS.BRDPoS)
	} else {
...
func (s *Service) reportBlock(conn *connWrapper, block *types.Block) error {
	...
	if (s.engine.(*BRDPoS.BRDPoS).EngineV2 != nil) && (s.engine.(*BRDPoS.BRDPoS).EngineV2.GetLatestCommittedBlockInfo() != nil) {
		latestCommittedBlockInfo := s.engine.(*BRDPoS.BRDPoS).EngineV2.GetLatestCommittedBlockInfo()
