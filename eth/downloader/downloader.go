...
// these are zero.
func (d *Downloader) Progress() BRDPoSChain.SyncProgress {
	...
	return BRDPoSChain.SyncProgress{
...
func (d *Downloader) processFullSyncContent(height uint64) error {
	for {
		...
		if d.blockchain.Config() != nil && d.blockchain.Config().BRDPoS != nil {
			epoch := d.blockchain.Config().BRDPoS.Epoch
			gap := d.blockchain.Config().BRDPoS.Gap
			inserts := []*fetchResult{}
