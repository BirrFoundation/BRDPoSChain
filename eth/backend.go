...
func New(ctx *node.ServiceContext, config *ethconfig.Config, BRCXServ *BRCx.BRCX, lendingServ *BRCxlending.Lending) (*Ethereum, error) {
	...
	// Inject BRCX Service into main Eth Service.
	if BRCXServ != nil {
		eth.BRCX = BRCXServ
	}
	if lendingServ != nil {
	...
	if eth.chainConfig.BRDPoS != nil {
		c := eth.engine.(*BRDPoS.BRDPoS)
		c.GetBRCXService = func() utils.TradingService {
			return eth.BRCX
		}
	...
	eth.blockchain, err = core.NewBlockChainEx(chainDb, BRCXServ.GetLevelDB(), cacheConfig, eth.chainConfig, eth.engine, vmConfig)
	if err != nil {
		return nil, err
	}
	...
	if eth.chainConfig.BRDPoS != nil {
		eth.ApiBackend = &EthApiBackend{eth, nil, eth.engine.(*BRDPoS.BRDPoS)}
	} else {
...
func CreateConsensusEngine(ctx *node.ServiceContext, config *ethash.Config, chainConfig *params.ChainConfig, db ethdb.Database) consensus.Engine {
	...
	if chainConfig.BRDPoS != nil {
		return BRDPoS.New(chainConfig, db)
	}
...
func (e *Ethereum) ValidateMasternode() (bool, error) {
	...
	if e.chainConfig.BRDPoS != nil {
		//check if miner's wallet is in set of validators
		c := e.engine.(*BRDPoS.BRDPoS)
		...
		return false, errors.New("only verify masternode permission in BRDPoS protocol")
	}
	return true, nil
}
...
func (e *Ethereum) StartStaking(local bool) error {
	...
	if BRDPoS, ok := e.engine.(*BRDPoS.BRDPoS); ok {
		...
		BRDPoS.Authorize(eb, wallet.SignHash)
	}
	if local {
...
func (e *Ethereum) GetBRCX() *BRCx.BRCX {
	return e.BRCX
}
...
func (e *Ethereum) GetBRCXLending() *BRCxlending.Lending {
	return e.Lending
}
