...
func NewProtocolManager(config *params.ChainConfig, mode downloader.SyncMode, networkID uint64, mux *event.TypeMux, txpool txPool, engine consensus.Engine, blockchain *core.BlockChain, chaindb ethdb.Database) (*ProtocolManager, error) {
	...
	if config.BRDPoS != nil {
		handleProposedBlock = func(header *types.Header) error {
			return engine.(*BRDPoS.BRDPoS).HandleProposedBlock(blockchain, header)
		}
	} else {
		handleProposedBlock = func(header *types.Header) error {
			return nil
	...
	if blockchain.Config().BRDPoS != nil {
...
type NodeInfo struct {
	Network    uint64              `json:"network"`    // BRC network ID (50=xinfin, 51=apothem, 551=devnet)
