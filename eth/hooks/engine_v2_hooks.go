...
func AttachConsensusV2Hooks(adaptor *BRDPoS.BRDPoS, bc *core.BlockChain, chainConfig *params.ChainConfig) {
	...
	adaptor.EngineV2.HookPenalty = func(chain consensus.ChainReader, number *big.Int, currentHash common.Hash, candidates []common.Address) ([]common.Address, error) {
		...
		comebackHeight := (common.LimitPenaltyEpochV2+1)*chain.Config().BRDPoS.Epoch + chain.Config().BRDPoS.V2.SwitchBlock.Uint64()
	...
	adaptor.EngineV2.HookReward = func(chain consensus.ChainReader, stateBlock *state.StateDB, parentState *state.StateDB, header *types.Header) (map[string]interface{}, error) {
		...
		if number == chain.Config().BRDPoS.V2.SwitchBlock.Uint64()+1 {
			return rewards, nil
		}
		start := time.Now()
...
func GetSigningTxCount(c *BRDPoS.BRDPoS, chain consensus.ChainReader, header *types.Header, totalSigner *uint64) (map[common.Address]*contracts.RewardLog, error) {
	...
	for i := number - 1; ; i-- {
		...
		if isEpochSwitch && i != chain.Config().BRDPoS.V2.SwitchBlock.Uint64()+1 {
			epochCount += 1
