...
func AttachConsensusV1Hooks(adaptor *BRDPoS.BRDPoS, bc *core.BlockChain, chainConfig *params.ChainConfig) {
	...
	adaptor.EngineV1.HookPenalty = func(chain consensus.ChainReader, blockNumberEpoc uint64) ([]common.Address, error) {
		...
		prevEpoc := blockNumberEpoc - chain.Config().BRDPoS.Epoch
	...
	adaptor.EngineV1.HookPenaltyTIPSigning = func(chain consensus.ChainReader, header *types.Header, candidates []common.Address) ([]common.Address, error) {
		prevEpoc := header.Number.Uint64() - chain.Config().BRDPoS.Epoch
		combackEpoch := uint64(0)
		comebackLength := (common.LimitPenaltyEpoch + 1) * chain.Config().BRDPoS.Epoch
		...
		listBlockHash := make([]common.Hash, chain.Config().BRDPoS.Epoch)
		...
		for i := uint64(1); i < chain.Config().BRDPoS.Epoch; i++ {
...
func getValidators(bc *core.BlockChain, masternodes []common.Address) ([]byte, error) {
	if bc.Config().BRDPoS == nil {
		return nil, core.ErrNotBRDPoS
	}
	client, err := bc.GetClient()
	if err != nil {
		return nil, err
