...
func (b *EthApiBackend) HeaderByNumber(ctx context.Context, blockNr rpc.BlockNumber) (*types.Header, error) {
	...
	if blockNr == rpc.LatestBlockNumber {
		...
		if b.eth.chainConfig.BRDPoS == nil {
			return nil, errors.New("PoW does not support confirmed block lookup")
		}
		current := b.eth.blockchain.CurrentBlock().Header()
		if b.eth.blockchain.Config().BRDPoS.BlockConsensusVersion(
			current.Number,
			current.Extra,
			BRDPoS.ExtraFieldCheck,
		) == params.ConsensusEngineVersion2 {
			// TO CHECK: why calling config in BRDPoS is blocked (not field and method)
			confirmedHash := b.BRDPoS.EngineV2.GetLatestCommittedBlockInfo().Hash
...
func (b *EthApiBackend) GetEVM(ctx context.Context, msg core.Message, state *state.StateDB, BRCxState *tradingstate.TradingStateDB, header *types.Header, vmConfig *vm.Config) (*vm.EVM, func() error, error) {
	...
	return vm.NewEVM(context, txContext, state, BRCxState, b.eth.chainConfig, *vmConfig), vmError, nil
}
...
func (b *EthApiBackend) GetVotersRewards(masternodeAddr common.Address) map[common.Address]*big.Int {
	...
	engine := b.GetEngine().(*BRDPoS.BRDPoS)
	foundationWalletAddr := chain.Config().BRDPoS.FoudationWalletAddr
	...
	chainReward := new(big.Int).Mul(new(big.Int).SetUint64(chain.Config().BRDPoS.Reward), new(big.Int).SetUint64(params.Ether))
...
func (b *EthApiBackend) GetEpochDuration() *big.Int {
	...
	block := chain.CurrentBlock()
	number := block.Number().Uint64()
	lastCheckpointNumber := number - (number % b.ChainConfig().BRDPoS.Epoch)
	lastCheckpointBlockTime := chain.GetBlockByNumber(lastCheckpointNumber).Time()
	secondToLastCheckpointNumber := lastCheckpointNumber - b.ChainConfig().BRDPoS.Epoch
...
func (b *EthApiBackend) GetOrderNonce(address common.Hash) (uint64, error) {
	BRCxService := b.eth.GetBRCX()
	if BRCxService != nil {
		author, err := b.GetEngine().Author(b.CurrentBlock().Header())
		if err != nil {
			return 0, err
		}
		BRCxState, err := BRCxService.GetTradingState(b.CurrentBlock(), author)
		if err != nil {
			return 0, err
		}
		return BRCxState.GetNonce(address), nil
	}
	return 0, errors.New("cannot find BRCx service")
}

func (b *EthApiBackend) BRCxService() *BRCx.BRCX {
	return b.eth.BRCX
}

func (b *EthApiBackend) LendingService() *BRCxlending.Lending {
