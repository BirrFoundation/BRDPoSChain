...
const (
	...
	/*
		...
		Hence the value of 200 has been chosen to safeguard the block/tx in BRC in terms of sizes.
...
func VerifyBalance(isBRCXLendingFork bool, statedb *state.StateDB, lendingStateDb *LendingStateDB,
	orderType, side, status string, userAddress, relayer, lendingToken, collateralToken common.Address,
	quantity, lendingTokenDecimal, collateralTokenDecimal, lendTokenBRCPrice, collateralPrice *big.Int,
	...
	case Market, Limit:
		...
		case Investing:
			...
			case LendingStatusNew:
				...
				if lendTokenBRCPrice != nil && lendTokenBRCPrice.Sign() > 0 {
					...
					var defaultFeeInBRC *big.Int
					if lendingToken != common.BRCNativeAddressBinary {
						defaultFeeInBRC = new(big.Int).Mul(defaultFee, lendTokenBRCPrice)
						defaultFeeInBRC = new(big.Int).Div(defaultFeeInBRC, lendingTokenDecimal)
					} else {
						defaultFeeInBRC = defaultFee
					}
					if defaultFeeInBRC.Cmp(common.RelayerLendingFee) <= 0 {
		...
		case Borrowing:
			...
			case LendingStatusNew:
				...
				settleBalanceResult, err := GetSettleBalance(isBRCXLendingFork, Borrowing, lendTokenBRCPrice, collateralPrice, depositRate, borrowingFeeRate, lendingToken, collateralToken, lendingTokenDecimal, collateralTokenDecimal, quantity)
				if err != nil {
					return err
				}
