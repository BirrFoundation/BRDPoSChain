...
func TestGetSettleBalance(t *testing.T) {
	...
	lendTokenNotBRC := common.HexToAddress("0x0000000000000000000000000000000000000033")
	...
	type GetSettleBalanceArg struct {
		isBRCXLendingFork      bool
		takerSide              string
		lendTokenBRCPrice      *big.Int
	...
	tests := []struct {
		...
		{
			"LendToken is BRC, quantity too small, taker BORROW",
			GetSettleBalanceArg{
				true,
				Borrowing,
				common.BasePrice,
		...
		{
			"LendToken is not BRC, quantity too small, taker BORROW",
			GetSettleBalanceArg{
				true,
				Borrowing,
				common.BasePrice,
		...
		{
			"LendToken is not BRC, no error, taker BORROW",
			GetSettleBalanceArg{
				true,
				Borrowing,
				common.BasePrice,
		...
		{
			"LendToken is not BRC, no error, taker INVEST",
			GetSettleBalanceArg{
				true,
				Investing,
				common.BasePrice,
		...
		{
			"LendToken is BRC, no error, taker invest",
			GetSettleBalanceArg{
				true,
				Investing,
				common.BasePrice,
			...
			&LendingSettleBalance{
				Taker: TradeResult{
					...
					InToken:  common.Address{},
					InTotal:  common.Big0,
					OutToken: common.BRCNativeAddressBinary,
					OutTotal: lendQuantity,
				},
				Maker: TradeResult{
					Fee:      fee,
					InToken:  common.BRCNativeAddressBinary,
					InTotal:  lendQuantityExcluded,
					OutToken: collateral,
					OutTotal: collateralLocked,
				},
		...
		{
			"LendToken is BRC, no error, taker Borrow",
			GetSettleBalanceArg{
				true,
				Borrowing,
				common.BasePrice,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := GetSettleBalance(tt.args.isBRCXLendingFork, tt.args.takerSide, tt.args.lendTokenBRCPrice, tt.args.collateralPrice, tt.args.depositRate, tt.args.borrowFeeRate, tt.args.lendingToken, tt.args.collateralToken, tt.args.lendTokenDecimal, tt.args.collateralTokenDecimal, tt.args.quantityToLend)
