...
const DefaultFeeRate = 100 // 100 / BRCXBaseFee = 100 / 10000 = 1%
var (
...
func GetSettleBalance(isBRCXLendingFork bool,
	takerSide string,
	lendTokenBRCPrice,
	collateralPrice,
	depositRate,
	...
	if !isBRCXLendingFork {
		if takerSide == Borrowing {
			...
			takerFee = new(big.Int).Div(takerFee, common.BRCXBaseFee)
			...
			if lendingToken != common.BRCNativeAddressBinary && lendTokenBRCPrice != nil && lendTokenBRCPrice.Cmp(common.Big0) > 0 {
				exTakerReceivedFee := new(big.Int).Mul(takerFee, lendTokenBRCPrice)
				exTakerReceivedFee = new(big.Int).Div(exTakerReceivedFee, lendTokenDecimal)

				defaultFeeInBRC := new(big.Int).Mul(defaultFee, lendTokenBRCPrice)
				defaultFeeInBRC = new(big.Int).Div(defaultFeeInBRC, lendTokenDecimal)

				if (exTakerReceivedFee.Cmp(common.RelayerLendingFee) <= 0 && exTakerReceivedFee.Sign() > 0) || defaultFeeInBRC.Cmp(common.RelayerLendingFee) <= 0 {
					log.Debug("takerFee too small", "quantityToLend", quantityToLend, "takerFee", takerFee, "exTakerReceivedFee", exTakerReceivedFee, "borrowFeeRate", borrowFeeRate, "defaultFeeInBRC", defaultFeeInBRC)
					return result, ErrQuantityTradeTooSmall
				}
			} else if lendingToken == common.BRCNativeAddressBinary {
			...
			if lendingToken != common.BRCNativeAddressBinary && lendTokenBRCPrice != nil && lendTokenBRCPrice.Cmp(common.Big0) > 0 {
				exMakerReceivedFee := new(big.Int).Mul(makerFee, lendTokenBRCPrice)
				exMakerReceivedFee = new(big.Int).Div(exMakerReceivedFee, lendTokenDecimal)

				defaultFeeInBRC := new(big.Int).Mul(defaultFee, lendTokenBRCPrice)
				defaultFeeInBRC = new(big.Int).Div(defaultFeeInBRC, lendTokenDecimal)

				if (exMakerReceivedFee.Cmp(common.RelayerLendingFee) <= 0 && exMakerReceivedFee.Sign() > 0) || defaultFeeInBRC.Cmp(common.RelayerLendingFee) <= 0 {
					log.Debug("makerFee too small", "quantityToLend", quantityToLend, "makerFee", makerFee, "exMakerReceivedFee", exMakerReceivedFee, "borrowFeeRate", borrowFeeRate, "defaultFeeInBRC", defaultFeeInBRC)
					return result, ErrQuantityTradeTooSmall
				}
			} else if lendingToken == common.BRCNativeAddressBinary {
		...
		borrowFee = new(big.Int).Div(borrowFee, common.BRCXBaseFee)
		...
		if lendingToken != common.BRCNativeAddressBinary && lendTokenBRCPrice != nil && lendTokenBRCPrice.Cmp(common.Big0) > 0 {
			...
			exReceivedFee := new(big.Int).Mul(borrowFee, lendTokenBRCPrice)
			exReceivedFee = new(big.Int).Div(exReceivedFee, lendTokenDecimal)

			defaultFeeInBRC := new(big.Int).Mul(defaultFee, lendTokenBRCPrice)
			defaultFeeInBRC = new(big.Int).Div(defaultFeeInBRC, lendTokenDecimal)

			if (exReceivedFee.Cmp(common.RelayerLendingFee) <= 0 && exReceivedFee.Sign() > 0) || defaultFeeInBRC.Cmp(common.RelayerLendingFee) <= 0 {
				log.Debug("takerFee too small", "quantityToLend", quantityToLend, "borrowFee", borrowFee, "exReceivedFee", exReceivedFee, "borrowFeeRate", borrowFeeRate, "defaultFeeInBRC", defaultFeeInBRC)
				return result, ErrQuantityTradeTooSmall
			}
		} else if lendingToken == common.BRCNativeAddressBinary {
