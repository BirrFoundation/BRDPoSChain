...
func New(BRCx *BRCx.BRCX) *Lending {
	lending := &Lending{
	...
	lending.StateCache = lendingstate.NewDatabase(BRCx.GetLevelDB())
	lending.BRCx = BRCx
	return lending
}

func (l *Lending) GetLevelDB() BRCxDAO.BRCXDAO {
	return l.BRCx.GetLevelDB()
}

func (l *Lending) GetMongoDB() BRCxDAO.BRCXDAO {
	return l.BRCx.GetMongoDB()
}
	...
	return []rpc.API{
		{
			...
			Service:   NewPublicBRCXLendingAPI(l),
			Public:    true,
		},
	}
}
...
func (l *Lending) RollbackLendingData(txhash common.Hash) error {
	...
	if items != nil {
		for _, item := range items.([]*lendingstate.LendingItem) {
			...
			log.Debug("BRCxlending reorg: rollback lendingItem", "txhash", txhash.Hex(), "item", lendingstate.ToJSON(item), "lendingItemHistory", cacheAtTxHash)
			if !ok || cacheAtTxHash == nil {
				log.Debug("BRCxlending reorg: remove item due to no lendingItemHistory", "item", lendingstate.ToJSON(item))
			...
			if (lendingItemHistory == lendingstate.LendingItemHistoryItem{}) {
				log.Debug("BRCxlending reorg: remove item due to empty lendingItemHistory", "item", lendingstate.ToJSON(item))
				if err := db.DeleteObject(item.Hash, &lendingstate.LendingItem{}); err != nil {
					return fmt.Errorf("failed to remove reorg LendingItem. Err: %v . Item: %s", err.Error(), lendingstate.ToJSON(item))
				}
				continue
			...
			log.Debug("BRCxlending reorg: update item to the last lendingItemHistory", "item", lendingstate.ToJSON(item), "lendingItemHistory", lendingItemHistory)
	...
	if items != nil {
		for _, trade := range items.([]*lendingstate.LendingTrade) {
			...
			log.Debug("BRCxlending reorg: rollback LendingTrade", "txhash", txhash.Hex(), "trade", lendingstate.ToJSON(trade), "LendingTradeHistory", cacheAtTxHash)
			if !ok || cacheAtTxHash == nil {
				log.Debug("BRCxlending reorg: remove trade due to no LendingTradeHistory", "trade", lendingstate.ToJSON(trade))
			...
			if (lendingTradeHistoryItem == lendingstate.LendingTradeHistoryItem{}) {
				log.Debug("BRCxlending reorg: remove trade due to empty LendingTradeHistory", "trade", lendingstate.ToJSON(trade))
				if err := db.DeleteObject(trade.Hash, &lendingstate.LendingTrade{}); err != nil {
					return fmt.Errorf("failed to remove reorg LendingTrade. Err: %v . Trade: %s", err.Error(), lendingstate.ToJSON(trade))
				}
				continue
			...
			log.Debug("BRCxlending reorg: update trade to the last lendingTradeHistoryItem", "trade", lendingstate.ToJSON(trade), "lendingTradeHistoryItem", lendingTradeHistoryItem)
			if err := db.PutObject(trade.Hash, trade); err != nil {
