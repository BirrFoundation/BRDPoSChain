...
func (l *Lending) processOrderList(header *types.Header, coinbase common.Address, chain consensus.ChainContext, statedb *state.StateDB, lendingStateDB *lendingstate.LendingStateDB, tradingStateDb *tradingstate.TradingStateDB, side string, lendingOrderBook common.Hash, Interest *big.Int, quantityStillToTrade *big.Int, order *lendingstate.LendingItem) (*big.Int, []*lendingstate.LendingTrade, []*lendingstate.LendingItem, error) {
	...
	for quantityToTrade.Sign() > 0 {
		...
		lendTokenBRCPrice, collateralPrice, err := l.GetCollateralPrices(header, chain, statedb, tradingStateDb, collateralToken, order.LendingToken)
		if err != nil {
			return nil, nil, nil, err
		}
		if lendTokenBRCPrice == nil || lendTokenBRCPrice.Sign() <= 0 {
		...
		tradedQuantity, collateralLockedAmount, rejectMaker, settleBalanceResult, err := l.getLendQuantity(lendTokenBRCPrice, collateralPrice, depositRate, borrowFee, coinbase, chain, header, statedb, order, &oldestOrder, maxTradedQuantity)
...
func (l *Lending) getLendQuantity(
	...
	LendingTokenDecimal, err := l.BRCx.GetTokenDecimal(chain, statedb, makerOrder.LendingToken)
	...
	collateralTokenDecimal, err := l.BRCx.GetTokenDecimal(chain, statedb, collateralToken)
	...
	if quantity.Sign() > 0 {
		// Apply Match Order
		isBRCXLendingFork := chain.Config().IsTIPBRCXLending(header.Number)
		settleBalanceResult, err := lendingstate.GetSettleBalance(isBRCXLendingFork, takerOrder.Side, lendTokenBRCPrice, collateralPrice, depositRate, borrowFee, lendToken, collateralToken, LendingTokenDecimal, collateralTokenDecimal, quantity)
...
func (l *Lending) ProcessCancelOrder(header *types.Header, lendingStateDB *lendingstate.LendingStateDB, statedb *state.StateDB, tradingStateDb *tradingstate.TradingStateDB, chain consensus.ChainContext, coinbase common.Address, lendingOrderBook common.Hash, order *lendingstate.LendingItem) (error, bool) {
	...
	lendTokenDecimal, err := l.BRCx.GetTokenDecimal(chain, statedb, originOrder.LendingToken)
	...
	if originOrder.Side == lendingstate.Borrowing {
		...
		collateralTokenDecimal, err = l.BRCx.GetTokenDecimal(chain, statedb, originOrder.CollateralToken)
	...
	if !chain.Config().IsTIPBRCXCancellationFee(header.Number) {
		...
		tokenCancelFee, tokenPriceInBRC = l.getCancelFee(chain, statedb, tradingStateDb, &originOrder, feeRate)
	}
...
func (l *Lending) getCancelFee(chain consensus.ChainContext, statedb *state.StateDB, tradingStateDb *tradingstate.TradingStateDB, order *lendingstate.LendingItem, feeRate *big.Int) (*big.Int, *big.Int) {
	...
	var err error
	if order.Side == lendingstate.Investing {
		cancelFee, tokenPriceInBRC, err = l.BRCx.ConvertBRCToToken(chain, statedb, tradingStateDb, order.LendingToken, common.RelayerLendingCancelFee)
	} else {
		cancelFee, tokenPriceInBRC, err = l.BRCx.ConvertBRCToToken(chain, statedb, tradingStateDb, order.CollateralToken, common.RelayerLendingCancelFee)
	}
	if err != nil {
		return common.Big0, common.Big0
	}
...
func (l *Lending) GetMediumTradePriceBeforeEpoch(chain consensus.ChainContext, statedb *state.StateDB, tradingStateDb *tradingstate.TradingStateDB, baseToken common.Address, quoteToken common.Address) (*big.Int, error) {
	...
	if price != nil && price.Sign() > 0 {
		...
		if inversePrice != nil && inversePrice.Sign() > 0 {
			quoteTokenDecimal, err := l.BRCx.GetTokenDecimal(chain, statedb, quoteToken)
			if err != nil || quoteTokenDecimal.Sign() == 0 {
				return nil, fmt.Errorf("fail to get tokenDecimal: Token: %v . Err: %v", quoteToken.String(), err)
			}
			baseTokenDecimal, err := l.BRCx.GetTokenDecimal(chain, statedb, baseToken)
...
// - Have direct pair in BRCX: lendToken/CollateralToken or CollateralToken/LendToken
// - Have pairs with BRC:
// -  lendToken/BRC and CollateralToken/BRC
// -  BRC/lendToken and BRC/CollateralToken
func (l *Lending) GetCollateralPrices(header *types.Header, chain consensus.ChainContext, statedb *state.StateDB, tradingStateDb *tradingstate.TradingStateDB, collateralToken common.Address, lendingToken common.Address) (*big.Int, *big.Int, error) {
	// lendTokenBRCPrice: price of ticker lendToken/BRC
	// collateralBRCPrice: price of ticker collateralToken/BRC
	...
	collateralPriceUpdatedFromContract := updatedBlock.Uint64()/chain.Config().BRDPoS.Epoch == header.Number.Uint64()/chain.Config().BRDPoS.Epoch

	lendTokenBRCPrice, err := l.GetBRCBasePrices(header, chain, statedb, tradingStateDb, lendingToken)
	if err != nil {
		return nil, nil, err
	}
	if collateralPriceUpdatedFromContract {
		...
		return lendTokenBRCPrice, collateralPriceFromContract, nil
	}
	lendingTokenDecimal, err := l.BRCx.GetTokenDecimal(chain, statedb, lendingToken)
	...
	inverseCollateralPriceUpdatedFromContract := updatedBlock.Uint64()/chain.Config().BRDPoS.Epoch == header.Number.Uint64()/chain.Config().BRDPoS.Epoch
	if inverseCollateralPriceUpdatedFromContract {
		...
		return lendTokenBRCPrice, collateralPrice, nil
	}
	// if contract doesn't provide any price information
	// getting price from pair in BRCx
	...
	if lastAveragePrice != nil && lastAveragePrice.Sign() > 0 {
		log.Debug("Getting collateral/lending from direct pair in BRCx", "lendToken", lendingToken.Hex(), "collateralToken", collateralToken.Hex(), "price", lastAveragePrice)
		return lendTokenBRCPrice, lastAveragePrice, nil
	}
	collateralBRCPrice, err := l.GetBRCBasePrices(header, chain, statedb, tradingStateDb, collateralToken)
	if err != nil {
		return nil, nil, err
	}
	if collateralBRCPrice == nil || lendTokenBRCPrice == nil {
		return common.Big0, common.Big0, nil
	}
	// Calculate collateral/LendToken price from collateral/BRC, lendToken/BRC
	collateralPrice = new(big.Int).Mul(collateralBRCPrice, lendingTokenDecimal)
	collateralPrice = new(big.Int).Div(collateralPrice, lendTokenBRCPrice)
	log.Debug("GetCollateralPrices: Calculate collateral/LendToken price from collateral/BRC, lendToken/BRC", "collateralPrice", collateralPrice,
		"collateralBRCPrice", collateralBRCPrice, "lendingTokenDecimal", lendingTokenDecimal, "lendTokenBRCPrice", lendTokenBRCPrice)
	return lendTokenBRCPrice, collateralPrice, nil
}

func (l *Lending) GetBRCBasePrices(header *types.Header, chain consensus.ChainContext, statedb *state.StateDB, tradingStateDb *tradingstate.TradingStateDB, token common.Address) (*big.Int, error) {

	tokenBRCPriceFromContract, updatedBlock := lendingstate.GetCollateralPrice(statedb, token, common.BRCNativeAddressBinary)
	tokenBRCPriceUpdatedFromContract := updatedBlock.Uint64()/chain.Config().BRDPoS.Epoch == header.Number.Uint64()/chain.Config().BRDPoS.Epoch

	if token == common.BRCNativeAddressBinary {
		return common.BasePrice, nil
	} else if tokenBRCPriceUpdatedFromContract {
		// getting lendToken price from contract first
		// otherwise, getting from BRCx lendToken/BRC
		log.Debug("Getting token/BRC price from contract", "price", tokenBRCPriceFromContract)
		return tokenBRCPriceFromContract, nil
	} else {
		BRCTokenPriceFromContract, updatedBlock := lendingstate.GetCollateralPrice(statedb, common.BRCNativeAddressBinary, token)
		BRCTokenPriceUpdatedFromContract := updatedBlock.Uint64()/chain.Config().BRDPoS.Epoch == header.Number.Uint64()/chain.Config().BRDPoS.Epoch
		if BRCTokenPriceUpdatedFromContract && BRCTokenPriceFromContract != nil && BRCTokenPriceFromContract.Sign() > 0 {
			// getting lendToken price from contract first
			// otherwise, getting from BRCx lendToken/BRC
			log.Debug("Getting BRC/token from contract", "price", BRCTokenPriceFromContract)
			tokenDecimal, err := l.BRCx.GetTokenDecimal(chain, statedb, token)
			log.Debug("GetTokenDecimal", "token", token.Hex(), "err", err)
			if err != nil || tokenDecimal == nil || tokenDecimal.Sign() == 0 {
				return nil, err
			}
			tokenBRCPrice := new(big.Int).Mul(common.BasePrice, tokenDecimal)
			tokenBRCPrice = new(big.Int).Div(tokenBRCPrice, BRCTokenPriceFromContract)
			return tokenBRCPrice, nil
		}
		tokenBRCPrice, err := l.GetMediumTradePriceBeforeEpoch(chain, statedb, tradingStateDb, token, common.BRCNativeAddressBinary)
		if err != nil {
			return nil, err
		}
		if tokenBRCPrice != nil && tokenBRCPrice.Sign() > 0 {
			log.Debug("Getting token/BRC from BRCx", "price", tokenBRCPrice, "err", err)
			return tokenBRCPrice, nil
		}
	}
	log.Debug("Can't getting tokenBRCPrice ", "token", token.Hex())
	return nil, nil
}
...
func (l *Lending) ProcessRepayLendingTrade(header *types.Header, chain consensus.ChainContext, lendingStateDB *lendingstate.LendingStateDB, statedb *state.StateDB, tradingstateDB *tradingstate.TradingStateDB, lendingBook common.Hash, lendingTradeId uint64) (trade *lendingstate.LendingTrade, err error) {
	...
	if tokenBalance.Cmp(paymentBalance) < 0 {
		...
		var err error
		if chain.Config().IsTIPBRCXLending(header.Number) {
