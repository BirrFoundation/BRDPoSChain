...
params=""
accountsCount=$(
  BRC account list --datadir $DATA_DIR  --keystore $KEYSTORE_DIR \
  2> /dev/null \
  | wc -l
)
...
if [[ ! -z $NETWORK_ID ]]; then
  case $NETWORK_ID in
    ...
    89 )
      ...
      params="$params --apothem --gcmode archive --http-api db,eth,net,web3,debug,BRDPoS"
      ;;
    90 )
...
# data dir
if [[ ! -d $DATA_DIR/BRC ]]; then
  echo "No blockchain data, creating genesis block."
  BRC init $genesisPath --datadir $DATA_DIR 2> /dev/null
fi
...
if [[ $accountsCount -le 0 ]]; then
  ...
  if [[ ! -z $PRIVATE_KEY ]]; then
    ...
    BRC  account import ./private_key \
      --datadir $DATA_DIR \
...
if [[ ! -z $DEBUG_MODE ]]; then
  params="$params --gcmode archive --http-api db,eth,net,web3,debug,BRDPoS"
fi
