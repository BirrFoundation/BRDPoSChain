...
import (
	...
	brc_genesis "BRDPoSChain/genesis"
	"BRDPoSChain/log"
...
func initGenesis(ctx *cli.Context) error {
	...
	if ctx.Bool(utils.MainnetFlag.Name) {
		...
		err = json.Unmarshal(brc_genesis.TestnetGenesis, &genesis)
		...
		if genesisPath == "mainnet" || genesisPath == "xinfin" {
			err = json.Unmarshal(brc_genesis.MainnetGenesis, &genesis)
		} else if genesisPath == "testnet" || genesisPath == "apothem" {
			err = json.Unmarshal(brc_genesis.TestnetGenesis, &genesis)
		} else if genesisPath == "devnet" {
			err = json.Unmarshal(brc_genesis.DevnetGenesis, &genesis)
		} else {
