...
import (
	...
	"BRDPoSChain/accounts/scwallet"
	"BRDPoSChain/accounts/usbwallet"
...
func loadConfig(file string, cfg *BRCConfig) error {
	f, err := os.Open(file)
	if err != nil {
		return err
	}
...
func makeConfigNode(ctx *cli.Context) (*node.Node, BRCConfig) {
	// Load defaults.
	cfg := BRCConfig{
		Eth:       ethconfig.Defaults,
		BRCX:      BRCx.DefaultConfig,
	...
	if ctx.Bool(utils.TestnetFlag.Name) {
		...
		common.TIPBRCXCancellationFee = common.TIPBRCXCancellationFeeTestnet
	}

	if ctx.Bool(utils.EnableBRCPrefixFlag.Name) {
	...
	utils.SetBRCXConfig(ctx, &cfg.BRCX, cfg.Node.DataDir)
...
func makeFullNode(ctx *cli.Context) (*node.Node, BRCConfig) {
	stack, cfg := makeConfigNode(ctx)

	// Start metrics export if enabled
	utils.SetupMetrics(&cfg.Metrics)

	// Register BRCX's OrderBook service if requested.
	// enable in default
	utils.RegisterBRCXService(stack, &cfg.BRCX)
...
func applyMetricConfig(ctx *cli.Context, cfg *BRCConfig) {
