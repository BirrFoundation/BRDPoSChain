...
func TestDAOForkBlockNewChain(t *testing.T) {
	for i, arg := range []struct {
		...
		// {"", params.BRCMainnetChainConfig.DAOForkBlock, false},
...
func testDAOForkBlockNewChain(t *testing.T, test int, genesis string, expectBlock *big.Int, expectVote bool) {
	...
	// Start a BRC instance with the requested flags set and immediately terminate
	if genesis != "" {
		json := filepath.Join(datadir, "genesis.json")
		if err := os.WriteFile(json, []byte(genesis), 0600); err != nil {
			t.Fatalf("test %d: failed to write genesis file: %v", test, err)
		}
		runBRC(t, "init", "--datadir", datadir, json).WaitExit()
	} else {
		// Force chain initialization
		BRC := runBRC(t, "console", "--port", "0", "--maxpeers", "0", "--nodiscover", "--nat", "none", "--ipcdisable", "--datadir", datadir, "--exec", "2+2")
		BRC.WaitExit()
	}
	// Retrieve the DAO config flag from the database
	path := filepath.Join(datadir, "BRC", "chaindata")
