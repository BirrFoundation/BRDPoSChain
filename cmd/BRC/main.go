...
import (
	...
	_ "BRDPoSChain/eth/tracers/native"
)

const (
	clientIdentifier = "BRC" // Client identifier to advertise over the network
)
...
var (
	...
	app = flags.NewApp(gitCommit, "the BRDPoSChain command line interface")
	...
	nodeFlags = []cli.Flag{
		...
		utils.BRCXEnabledFlag,
		utils.BRCXDataDirFlag,
		utils.BRCXDBEngineFlag,
		utils.BRCXDBConnectionUrlFlag,
		utils.BRCXDBReplicaSetNameFlag,
		utils.BRCXDBNameFlag,
...
func init() {
	// Initialize the CLI app and start BRC
	app.Action = BRC
	app.Copyright = "Copyright (c) 2024 BRDPoSChain"
	...
	flags.AutoEnvVars(app.Flags, "BRC")
	...
	app.Before = func(ctx *cli.Context) error {
		...
		flags.CheckEnvVars(ctx, app.Flags, "BRC")
...
// BRC is the main entry point into the system if no special subcommand is ran.
// It creates a default node based on the command line arguments and runs it in
// blocking mode, waiting for it to be shut down.
func BRC(ctx *cli.Context) error {
...
// miner.
func startNode(ctx *cli.Context, stack *node.Node, cfg BRCConfig) {
	...
	if engine, ok := ethereum.Engine().(*BRDPoS.BRDPoS); ok {
		go func() {
			started := false
			ok := false
			slaveMode := ctx.IsSet(utils.BRCSlaveModeFlag.Name)
			var err error
