...
RUN \
  echo 'node server.js &'                     > wallet.sh && \
	echo 'BRC --cache 512 init /genesis.json' >> wallet.sh && \
	echo $'BRC --networkid {{.NetworkID}} --port {{.NodePort}} --bootnodes {{.Bootnodes}} --ethstats \'{{.Ethstats}}\' --cache=512 --http --http-addr=0.0.0.0 --http-corsdomain "*" --http-vhosts "*"' >> wallet.sh
