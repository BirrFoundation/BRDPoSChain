...
type NetworkInformation struct {
	NetworkId                  *big.Int
	BRCValidatorAddress        common.Address
	RelayerRegistrationAddress common.Address
	BRCXListingAddress         common.Address
	BRCZAddress                common.Address
	LendingAddress             common.Address
	ConsensusConfigs           params.BRDPoSConfig
}
...
func (api *API) GetSnapshot(number *rpc.BlockNumber) (*utils.PublicApiSnapshot, error) {
	...
	return api.BRDPoS.GetSnapshot(api.chain, header)
}
...
func (api *API) GetSigners(number *rpc.BlockNumber) ([]common.Address, error) {
	...
	return api.BRDPoS.GetAuthorisedSignersFromSnapshot(api.chain, header)
}
...
func (api *API) GetMasternodesByNumber(number *rpc.BlockNumber) MasternodesStatus {
	...
	if number == nil || *number == rpc.LatestBlockNumber {
		...
		hash := api.BRDPoS.EngineV2.GetLatestCommittedBlockInfo().Hash
		header = api.chain.GetHeaderByHash(hash)
	} else {
		header = api.chain.GetHeaderByNumber(uint64(number.Int64()))
	}

	round, err := api.BRDPoS.EngineV2.GetRoundNumber(header)
	if err != nil {
	...
	masterNodes := api.BRDPoS.EngineV2.GetMasternodes(api.chain, header)
	penalties := api.BRDPoS.EngineV2.GetPenalties(api.chain, header)
	standbynodes := api.BRDPoS.EngineV2.GetStandbynodes(api.chain, header)
...
func (api *API) GetLatestPoolStatus() MessageStatus {
	...
	receivedVotes := api.BRDPoS.EngineV2.ReceivedVotes()
	receivedTimeouts := api.BRDPoS.EngineV2.ReceivedTimeouts()
...
func (api *API) GetV2BlockByHeader(header *types.Header, uncle bool) *V2BlockInfo {
	committed := false
	latestCommittedBlock := api.BRDPoS.EngineV2.GetLatestCommittedBlockInfo()
...
func (api *API) NetworkInformation() NetworkInformation {
	...
	info.BRCValidatorAddress = common.MasternodeVotingSMCBinary
	if common.IsTestnet {
		...
		info.BRCXListingAddress = common.BRCXListingSMCTestNet
		info.BRCZAddress = common.TRC21IssuerSMCTestNet
	} else {
		info.LendingAddress = common.LendingRegistrationSMC
		info.RelayerRegistrationAddress = common.RelayerRegistrationSMC
		info.BRCXListingAddress = common.BRCXListingSMC
		info.BRCZAddress = common.TRC21IssuerSMC
	}
	info.ConsensusConfigs = *api.BRDPoS.config
...
func (api *API) GetMissedRoundsInEpochByBlockNum(number *rpc.BlockNumber) (*utils.PublicApiMissedRoundsMetadata, error) {
	return api.BRDPoS.CalculateMissingRounds(api.chain, api.getHeaderFromApiBlockNum(number))
}
...
func (api *API) GetEpochNumbersBetween(begin, end *rpc.BlockNumber) ([]uint64, error) {
	...
	epochSwitchInfos, err := api.BRDPoS.GetEpochSwitchInfoBetween(api.chain, beginHeader, endHeader)
	if err != nil {
		return nil, err
	}
...
func (api *API) GetBlockInfoByEpochNum(epochNumber uint64) (*utils.EpochNumInfo, error) {
	thisEpoch, err := api.BRDPoS.EngineV2.GetBlockByEpochNumber(api.chain, epochNumber)
	if err != nil {
		return nil, err
	}
	...
	nextEpoch, err := api.BRDPoS.EngineV2.GetBlockByEpochNumber(api.chain, epochNumber+1)
