...
type SnapshotV1 struct {
	config   *params.BRDPoSConfig // Consensus engine parameters to fine tune behavior
...
// the genesis block.
func newSnapshot(config *params.BRDPoSConfig, sigcache *utils.SigLRU, number uint64, hash common.Hash, signers []common.Address) *SnapshotV1 {
	snap := &SnapshotV1{
		config:   config,
		sigcache: sigcache,
		Number:   number,
...
func loadSnapshot(config *params.BRDPoSConfig, sigcache *utils.SigLRU, db ethdb.Database, hash common.Hash) (*SnapshotV1, error) {
	blob, err := db.Get(append([]byte("BRDPoS-"), hash[:]...))
	if err != nil {
		return nil, err
	}
	snap := new(SnapshotV1)
...
func (s *SnapshotV1) store(db ethdb.Database) error {
	...
	return db.Put(append([]byte("BRDPoS-"), s.Hash[:]...), blob)
}
