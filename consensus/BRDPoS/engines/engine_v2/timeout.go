...
In the engine v2, we will need to:
 ...
 3. generateSyncInfo()
*/
func (x *BRDPoS_v2) onTimeoutPoolThresholdReached(blockChainReader consensus.ChainReader, pooledTimeouts map[common.Hash]utils.PoolObj, currentTimeoutMsg utils.PoolObj, gapNumber uint64) error {
...
func (x *BRDPoS_v2) getTCEpochInfo(chain consensus.ChainReader, timeoutCert *types.TimeoutCert) (*types.EpochSwitchInfo, error) {
...
func (x *BRDPoS_v2) verifyTC(chain consensus.ChainReader, timeoutCert *types.TimeoutCert) error {
	/*
		...
		2. Verify signer signature: (List of signatures)
					...
					- Use the above brc address to check against the master node list from step 1(For the received TC epoch)
	*/
...
*/
func (x *BRDPoS_v2) processTC(blockChainReader consensus.ChainReader, timeoutCert *types.TimeoutCert) error {
...
*/
func (x *BRDPoS_v2) sendTimeout(chain consensus.ChainReader) error {
...
*/
func (x *BRDPoS_v2) OnCountdownTimeout(time time.Time, chain interface{}) error {
	x.lock.Lock()
	defer x.lock.Unlock()
...
func (x *BRDPoS_v2) hygieneTimeoutPool() {
	x.lock.RLock()
	currentRound := x.currentRound
	x.lock.RUnlock()
...
func (x *BRDPoS_v2) ReceivedTimeouts() map[string]map[common.Hash]utils.PoolObj {
