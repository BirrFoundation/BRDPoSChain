...
func TestAdaptorGetMasternodesFromCheckpointHeader(t *testing.T) {
	blockchain, _, currentBlock, _, _, _ := PrepareBRCTestBlockChainForV2Engine(t, 1, params.TestBRDPoSMockChainConfig, nil)
	adaptor := blockchain.Engine().(*BRDPoS.BRDPoS)
	...
	headerV2.Number.Add(blockchain.Config().BRDPoS.V2.SwitchBlock, big.NewInt(1))
...
func TestAdaptorIsEpochSwitch(t *testing.T) {
	...
	parentBlockInfo := &types.BlockInfo{
		...
		Round:  types.Round(0),
		Number: big.NewInt(0).Set(blockchain.Config().BRDPoS.V2.SwitchBlock),
	}
	...
	header.Number.Add(blockchain.Config().BRDPoS.V2.SwitchBlock, big.NewInt(1))
	isEpochSwitchBlock, _, err = adaptor.IsEpochSwitch(header)
	assert.Nil(t, err)
	assert.True(t, isEpochSwitchBlock, "header should be epoch switch", header)
	parentBlockInfo = &types.BlockInfo{
		Hash:   header.ParentHash,
		Round:  types.Round(1),
		Number: big.NewInt(0).Add(blockchain.Config().BRDPoS.V2.SwitchBlock, big.NewInt(1)),
	}
	quorumCert = &types.QuorumCert{
		ProposedBlockInfo: parentBlockInfo,
		Signatures:        nil,
	...
	assert.Nil(t, err)
	header.Extra = extraBytes
	header.Number.Add(blockchain.Config().BRDPoS.V2.SwitchBlock, big.NewInt(2))
	isEpochSwitchBlock, _, err = adaptor.IsEpochSwitch(header)
	assert.Nil(t, err)
	assert.False(t, isEpochSwitchBlock, "header should not be epoch switch", header)
	parentBlockInfo = &types.BlockInfo{
		Hash:   header.ParentHash,
		Round:  types.Round(blockchain.Config().BRDPoS.Epoch) - 1,
		Number: big.NewInt(0).Add(blockchain.Config().BRDPoS.V2.SwitchBlock, big.NewInt(100)),
	}
	quorumCert = &types.QuorumCert{
		ProposedBlockInfo: parentBlockInfo,
		Signatures:        nil,
	...
	assert.Nil(t, err)
	header.Extra = extraBytes
	header.Number.Add(blockchain.Config().BRDPoS.V2.SwitchBlock, big.NewInt(101))
	isEpochSwitchBlock, _, err = adaptor.IsEpochSwitch(header)
	assert.Nil(t, err)
	assert.True(t, isEpochSwitchBlock, "header should be epoch switch", header)
	parentBlockInfo = &types.BlockInfo{
	...
	extra = types.ExtraFields_v2{
		Round:      types.Round(blockchain.Config().BRDPoS.Epoch) + 2,
		QuorumCert: quorumCert,
	}
	extraBytes, err = extra.EncodeToBytes()
	assert.Nil(t, err)
...
func TestGetParentBlock(t *testing.T) {
	blockchain, _, block900, signer, signFn, _ := PrepareBRCTestBlockChainForV2Engine(t, 900, params.TestBRDPoSMockChainConfig, nil)
	adaptor := blockchain.Engine().(*BRDPoS.BRDPoS)
	...
	blockNum := 901
	blockCoinBase := "0x111000000000000000000000000000000123"
	block901 := CreateBlock(blockchain, params.TestBRDPoSMockChainConfig, block900, blockNum, 1, blockCoinBase, signer, signFn, nil, nil, "")
	err = blockchain.InsertBlock(block901)
	assert.Nil(t, err)

	// let's inject another one, but the highestedQC has not been updated, so it shall still point to 900
	blockNum = 902
	block902 := CreateBlock(blockchain, params.TestBRDPoSMockChainConfig, block901, blockNum, 1, blockCoinBase, signer, signFn, nil, nil, "")
