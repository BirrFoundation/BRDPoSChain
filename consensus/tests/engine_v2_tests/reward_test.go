...
func TestHookRewardV2(t *testing.T) {
	...
	config.BRDPoS.V2.SwitchBlock.SetUint64(1800)

	blockchain, _, _, signer, signFn, _ := PrepareBRCTestBlockChainForV2Engine(t, int(config.BRDPoS.Epoch)*5, &config, nil)
	...
	header915 := blockchain.GetHeaderByNumber(config.BRDPoS.Epoch + 15)
	header916 := blockchain.GetHeaderByNumber(config.BRDPoS.Epoch + 16)
	header1799 := blockchain.GetHeaderByNumber(config.BRDPoS.Epoch*2 - 1)
	header1801 := blockchain.GetHeaderByNumber(config.BRDPoS.Epoch*2 + 1)
	...
	header2699 := blockchain.GetHeaderByNumber(config.BRDPoS.Epoch*3 - 1)
	header2700 := blockchain.GetHeaderByNumber(config.BRDPoS.Epoch * 3)
	...
	header2685 := blockchain.GetHeaderByNumber(config.BRDPoS.Epoch*2 + 885)
	header2716 := blockchain.GetHeaderByNumber(config.BRDPoS.Epoch*3 + 16)
	header3599 := blockchain.GetHeaderByNumber(config.BRDPoS.Epoch*4 - 1)
	header3600 := blockchain.GetHeaderByNumber(config.BRDPoS.Epoch * 4)
	...
	for _, x := range result {
		...
		assert.Zero(t, a.Cmp(r[owner]))
		b, _ := big.NewInt(0).SetString("25000000000000000000", 10)
		assert.Zero(t, b.Cmp(r[config.BRDPoS.FoudationWalletAddr]))
	}
	// if no signing tx, then reward will be 0
	header4499 := blockchain.GetHeaderByNumber(config.BRDPoS.Epoch*5 - 1)
	header4500 := blockchain.GetHeaderByNumber(config.BRDPoS.Epoch * 5)
...
func TestHookRewardV2SplitReward(t *testing.T) {
	...
	blockchain, _, _, signer, signFn, _ := PrepareBRCTestBlockChainForV2Engine(t, int(config.BRDPoS.Epoch)*3, &config, nil)
	...
	header915 := blockchain.GetHeaderByNumber(config.BRDPoS.Epoch + 15)
	header916 := blockchain.GetHeaderByNumber(config.BRDPoS.Epoch + 16)
	// header917 := blockchain.GetHeaderByNumber(config.BRDPoS.Epoch + 17)
	header1785 := blockchain.GetHeaderByNumber(config.BRDPoS.Epoch*2 - 15)
	header1799 := blockchain.GetHeaderByNumber(config.BRDPoS.Epoch*2 - 1)
	header1801 := blockchain.GetHeaderByNumber(config.BRDPoS.Epoch*2 + 1)
	tx, err := signingTxWithSignerFn(header915, 0, signer, signFn)
	assert.Nil(t, err)
