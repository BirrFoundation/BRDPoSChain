...
func TestProcessQcShallSetForensicsCommittedQc(t *testing.T) {
	...
	blockchain, _, currentBlock, signer, signFn, _ := PrepareBRCTestBlockChainForV2Engine(t, 905, params.TestBRDPoSMockChainConfig, nil)
...
func TestSetCommittedQCsInOrder(t *testing.T) {
	...
	blockchain, _, currentBlock, _, _, _ := PrepareBRCTestBlockChainForV2Engine(t, 905, params.TestBRDPoSMockChainConfig, nil)
	forensics := blockchain.Engine().(*BRDPoS.BRDPoS).EngineV2.GetForensicsFaker()
...
func TestForensicsMonitoring(t *testing.T) {
	...
	blockchain, _, currentBlock, _, _, _ := PrepareBRCTestBlockChainForV2Engine(t, 915, params.TestBRDPoSMockChainConfig, nil)
	forensics := blockchain.Engine().(*BRDPoS.BRDPoS).EngineV2.GetForensicsFaker()
	...
	err = forensics.ForensicsMonitoring(blockchain, blockchain.Engine().(*BRDPoS.BRDPoS).EngineV2, newIncomingQcHeaders, *incomingQC)
	assert.Nil(t, err)
}
...
func TestForensicsMonitoringNotOnSameChainButHaveSameRoundQC(t *testing.T) {
	...
	*forkRoundDifference = 1
	blockchain, _, _, _, _, currentForkBlock := PrepareBRCTestBlockChainForV2Engine(t, 915, params.TestBRDPoSMockChainConfig, &ForkedBlockOptions{numOfForkedBlocks: numOfForks, forkedRoundDifference: forkRoundDifference})
	forensics := blockchain.Engine().(*BRDPoS.BRDPoS).EngineV2.GetForensicsFaker()
	...
	err = forensics.ForensicsMonitoring(blockchain, blockchain.Engine().(*BRDPoS.BRDPoS).EngineV2, forkedHeaders, *incomingQC)
	assert.Nil(t, err)
...
func TestForensicsMonitoringNotOnSameChainDoNotHaveSameRoundQC(t *testing.T) {
	...
	*forkRoundDifference = 10
	...
	blockchain, _, _, _, _, currentForkBlock := PrepareBRCTestBlockChainForV2Engine(t, 915, params.TestBRDPoSMockChainConfig, &ForkedBlockOptions{numOfForkedBlocks: numOfForks, forkedRoundDifference: forkRoundDifference, signersKey: forkedChainSignersKey})
	forensics := blockchain.Engine().(*BRDPoS.BRDPoS).EngineV2.GetForensicsFaker()
...
func TestForensicsAcrossEpoch(t *testing.T) {
	...
	*forkRoundDifference = 10
	var forkedChainSignersKey []*ecdsa.PrivateKey
	forkedChainSignersKey = append(forkedChainSignersKey, acc1Key)
	blockchain, _, _, _, _, currentForkBlock := PrepareBRCTestBlockChainForV2Engine(t, 1801, params.TestBRDPoSMockChainConfig, &ForkedBlockOptions{numOfForkedBlocks: numOfForks, forkedRoundDifference: forkRoundDifference, signersKey: forkedChainSignersKey})
	forensics := blockchain.Engine().(*BRDPoS.BRDPoS).EngineV2.GetForensicsFaker()
...
func TestVoteEquivocationSameRound(t *testing.T) {
	...
	var numOfForks = new(int)
	*numOfForks = 1
	blockchain, _, currentBlock, signer, signFn, currentForkBlock := PrepareBRCTestBlockChainForV2Engine(t, 901, params.TestBRDPoSMockChainConfig, &ForkedBlockOptions{numOfForkedBlocks: numOfForks})
	engineV2 := blockchain.Engine().(*BRDPoS.BRDPoS).EngineV2
	// Set up forensics events trigger
	forensics := blockchain.Engine().(*BRDPoS.BRDPoS).EngineV2.GetForensicsFaker()
	forensicsEventCh := make(chan types.ForensicsEvent)
...
func TestVoteEquivocationDifferentRound(t *testing.T) {
	...
	*forkRoundDifference = 1
	...
	err = forensics.ProcessVoteEquivocation(blockchain, blockchain.Engine().(*BRDPoS.BRDPoS).EngineV2, incomingVote)
	assert.Nil(t, err)
	// Check SendForensicProof triggered
	for {
		select {
