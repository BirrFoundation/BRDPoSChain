...
func TestGetMissedRoundsInEpochByBlockNumOnlyForV2Consensus(t *testing.T) {
	_, bc, _, _, _ := PrepareBRCTestBlockChainWith128Candidates(t, 1802, params.TestBRDPoSMockChainConfig)

	engine := bc.BlockChain().Engine().(*BRDPoS.BRDPoS)
	blockNum := rpc.BlockNumber(123)

	data, err := engine.APIs(bc.BlockChain())[0].Service.(*BRDPoS.API).GetMissedRoundsInEpochByBlockNum(&blockNum)
...
func TestGetMissedRoundsInEpochByBlockNumReturnEmptyForV2(t *testing.T) {
	_, bc, cb, _, _ := PrepareBRCTestBlockChainWith128Candidates(t, 1802, params.TestBRDPoSMockChainConfig)
...
func TestGetMissedRoundsInEpochByBlockNum(t *testing.T) {
	blockchain, bc, currentBlock, signer, signFn := PrepareBRCTestBlockChainWith128Candidates(t, 1802, params.TestBRDPoSMockChainConfig)
	chainConfig := params.TestBRDPoSMockChainConfig
	engine := bc.BlockChain().Engine().(*BRDPoS.BRDPoS)
	blockCoinBase := signer.Hex()

	startingBlockNum := currentBlock.Number().Int64() + 1
	// Skipped the round
	roundNumber := startingBlockNum - chainConfig.BRDPoS.V2.SwitchBlock.Int64() + 2
...
func TestGetEpochNumbersBetween(t *testing.T) {
	...
	numbers, err := engine.APIs(bc.BlockChain())[0].Service.(*BRDPoS.API).GetEpochNumbersBetween(&begin, &end)
...
func TestGetBlockByEpochNumber(t *testing.T) {
	blockchain, _, currentBlock, signer, signFn := PrepareBRCTestBlockChainWithPenaltyForV2Engine(t, 1802, params.TestBRDPoSMockChainConfig)

	blockCoinBase := "0x111000000000000000000000000000000123"
	largeRound := int64(1802)
	newBlock := CreateBlock(blockchain, params.TestBRDPoSMockChainConfig, currentBlock, int(currentBlock.NumberU64())+1, largeRound, blockCoinBase, signer, signFn, nil, nil, currentBlock.Header().Root.Hex())
	err := blockchain.InsertBlock(newBlock)
	assert.Nil(t, err)
	largeRound2 := int64(3603)
	newBlock2 := CreateBlock(blockchain, params.TestBRDPoSMockChainConfig, newBlock, int(newBlock.NumberU64())+1, largeRound2, blockCoinBase, signer, signFn, nil, nil, newBlock.Header().Root.Hex())
	...
	checkpointHeader := blockchain.GetHeaderByNumber(blockchain.Config().BRDPoS.V2.SwitchBlock.Uint64() + 1)
	err = engine.Initial(blockchain, checkpointHeader)
	assert.Nil(t, err)

	info, err := engine.APIs(blockchain)[0].Service.(*BRDPoS.API).GetBlockInfoByEpochNum(0)
	assert.NotNil(t, err)
	assert.Nil(t, info)

	info, err = engine.APIs(blockchain)[0].Service.(*BRDPoS.API).GetBlockInfoByEpochNum(1)
	assert.Equal(t, info.EpochRound, types.Round(1))
	assert.Nil(t, err)

	info, err = engine.APIs(blockchain)[0].Service.(*BRDPoS.API).GetBlockInfoByEpochNum(2)
	assert.Equal(t, info.EpochRound, types.Round(900))
	assert.Nil(t, err)

	info, err = engine.APIs(blockchain)[0].Service.(*BRDPoS.API).GetBlockInfoByEpochNum(3)
	assert.Equal(t, info.EpochRound, types.Round(largeRound))
	assert.Nil(t, err)

	info, err = engine.APIs(blockchain)[0].Service.(*BRDPoS.API).GetBlockInfoByEpochNum(4)
	assert.NotNil(t, err)
	assert.Nil(t, info)

	info, err = engine.APIs(blockchain)[0].Service.(*BRDPoS.API).GetBlockInfoByEpochNum(5)
	assert.Equal(t, info.EpochRound, types.Round(largeRound2))
	assert.Nil(t, err)

	info, err = engine.APIs(blockchain)[0].Service.(*BRDPoS.API).GetBlockInfoByEpochNum(6)
	assert.NotNil(t, err)
	assert.Nil(t, info)
}
