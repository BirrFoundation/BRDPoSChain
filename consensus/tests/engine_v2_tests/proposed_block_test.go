...
func TestShouldSendVoteMsgAndCommitGrandGrandParentBlock(t *testing.T) {
	...
	blockNum = 903
	blockCoinBase = fmt.Sprintf("0x111000000000000000000000000000000%03d", blockNum)
	block903 := CreateBlock(blockchain, params.TestBRDPoSMockChainConfig, block902, blockNum, 3, blockCoinBase, signer, signFn, nil, nil, "")
	...
	blockNum = 904
	blockCoinBase = fmt.Sprintf("0x111000000000000000000000000000000%03d", blockNum)
	block904 := CreateBlock(blockchain, params.TestBRDPoSMockChainConfig, block903, blockNum, 4, blockCoinBase, signer, signFn, nil, nil, "")
...
func TestShouldNotCommitIfRoundsNotContinousFor3Rounds(t *testing.T) {
	...
	blockNum := 906
	blockCoinBase := fmt.Sprintf("0x111000000000000000000000000000000%03d", blockNum)
	block906 := CreateBlock(blockchain, params.TestBRDPoSMockChainConfig, currentBlock, blockNum, 7, blockCoinBase, signer, signFn, nil, nil, "")
	...
	blockNum = 907
	blockCoinBase = fmt.Sprintf("0x111000000000000000000000000000000%03d", blockNum)
	block907 := CreateBlock(blockchain, params.TestBRDPoSMockChainConfig, block906, blockNum, 8, blockCoinBase, signer, signFn, nil, nil, "")
...
func TestProposedBlockMessageHandlerSuccessfullyGenerateVote(t *testing.T) {
	blockchain, _, currentBlock, _, _, _ := PrepareBRCTestBlockChainForV2Engine(t, 906, params.TestBRDPoSMockChainConfig, nil)
	engineV2 := blockchain.Engine().(*BRDPoS.BRDPoS).EngineV2
...
func TestShouldNotSendVoteMsgIfBlockNotExtendedFromAncestor(t *testing.T) {
	...
	*numOfForks = 3
	blockchain, _, currentBlock, _, _, forkedBlock := PrepareBRCTestBlockChainForV2Engine(t, 906, params.TestBRDPoSMockChainConfig, &ForkedBlockOptions{numOfForkedBlocks: numOfForks})
	engineV2 := blockchain.Engine().(*BRDPoS.BRDPoS).EngineV2
...
func TestShouldSendVoteMsg(t *testing.T) {
	...
	blockchain, _, _, _, _, _ := PrepareBRCTestBlockChainForV2Engine(t, 903, params.TestBRDPoSMockChainConfig, nil)
	engineV2 := blockchain.Engine().(*BRDPoS.BRDPoS).EngineV2
