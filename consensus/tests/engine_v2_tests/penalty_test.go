...
func TestHookPenaltyV2Mining(t *testing.T) {
	...
	blockchain, _, _, _, _, _ := PrepareBRCTestBlockChainForV2Engine(t, int(config.BRDPoS.Epoch)*3, config, nil)
	...
	header901 := blockchain.GetHeaderByNumber(config.BRDPoS.Epoch + 1)
	...
	header2100 := blockchain.GetHeaderByNumber(config.BRDPoS.Epoch * 3)
	penalty, err := adaptor.EngineV2.HookPenalty(blockchain, big.NewInt(int64(config.BRDPoS.Epoch*3)), header2100.ParentHash, masternodes)
	assert.Nil(t, err)
	...
	// Hence int(config.BRDPoS.Epoch)*3+18-900, the +18 means is to force to next 18 round and -900 is the relative round number to block number int(config.BRDPoS.Epoch)*3
	adaptor.EngineV2.SetNewRoundFaker(blockchain, types.Round(int(config.BRDPoS.Epoch)*3+18-900), false)
	...
	// 20 candidates (set by PrepareBRCTestBlockChainForV2Engine) - 1 penalty = 19
	assert.Equal(t, 19, len(headerMining.Validators)/common.AddressLength)
}

func TestHookPenaltyV2Comeback(t *testing.T) {
	config := params.TestBRDPoSMockChainConfig
	blockchain, _, _, signer, signFn := PrepareBRCTestBlockChainWithPenaltyForV2Engine(t, int(config.BRDPoS.Epoch)*3, config)
	adaptor := blockchain.Engine().(*BRDPoS.BRDPoS)
	hooks.AttachConsensusV2Hooks(adaptor, blockchain, config)
	assert.NotNil(t, adaptor.EngineV2.HookPenalty)
	var extraField types.ExtraFields_v2
	...
	header2085 := blockchain.GetHeaderByNumber(config.BRDPoS.Epoch*3 - common.MergeSignRange)
...
func TestHookPenaltyV2Jump(t *testing.T) {
	config := params.TestBRDPoSMockChainConfig
	end := int(config.BRDPoS.Epoch)*3 - common.MergeSignRange
	blockchain, _, _, _, _ := PrepareBRCTestBlockChainWithPenaltyForV2Engine(t, int(config.BRDPoS.Epoch)*3, config)
	adaptor := blockchain.Engine().(*BRDPoS.BRDPoS)
	hooks.AttachConsensusV2Hooks(adaptor, blockchain, config)
	assert.NotNil(t, adaptor.EngineV2.HookPenalty)
	var extraField types.ExtraFields_v2
	...
	adaptor.EngineV2.SetNewRoundFaker(blockchain, types.Round(config.BRDPoS.Epoch*3), false)
