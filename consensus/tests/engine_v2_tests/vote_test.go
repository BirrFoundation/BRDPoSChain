...
func TestVoteMessageShallNotThrowErrorIfBlockNotYetExist(t *testing.T) {
	...
	block := CreateBlock(blockchain, params.TestBRDPoSMockChainConfig, currentBlock, blockNum, 6, blockCoinBase, signer, signFn, nil, nil, "")
...
func TestVerifyVoteMsg(t *testing.T) {
	blockchain, _, currentBlock, signer, signFn, _ := PrepareBRCTestBlockChainForV2Engine(t, 915, params.TestBRDPoSMockChainConfig, nil)
	engineV2 := blockchain.Engine().(*BRDPoS.BRDPoS).EngineV2
