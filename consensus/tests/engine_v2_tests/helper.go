...
var (
	...
	acc1Addr    = crypto.PubkeyToAddress(acc1Key.PublicKey)  //brc703c4b2bD70c169f5717101CaeE543299Fc946C7
	acc2Addr    = crypto.PubkeyToAddress(acc2Key.PublicKey)  //brc0D3ab14BBaD3D99F4203bd7a11aCB94882050E7e
	acc3Addr    = crypto.PubkeyToAddress(acc3Key.PublicKey)  //brc71562b71999873DB5b286dF957af199Ec94617F7
...
func getCommonBackend(t *testing.T, chainConfig *params.ChainConfig) *backends.SimulatedBackend {
	...
	contractBackendForSC := backends.NewBRCSimulatedBackend(types.GenesisAlloc{
	...
	validatorSCAddr, _, _, err := contractValidator.DeployBRCValidator(
		transactOpts,
	...
	contractBackend2 := backends.NewBRCSimulatedBackend(types.GenesisAlloc{
...
func GetSnapshotSigner(bc *core.BlockChain, header *types.Header) (signersList, error) {
	engine := bc.Engine().(*BRDPoS.BRDPoS)
...
func GetCandidateFromCurrentSmartContract(backend bind.ContractBackend, t *testing.T) masterNodes {
	...
	validator, err := contractValidator.NewBRCValidator(addr, backend)
	if err != nil {
		t.Fatal(err)
	}
...
// V2 concensus engine
func PrepareBRCTestBlockChainForV2Engine(t *testing.T, numOfBlocks int, chainConfig *params.ChainConfig, forkedBlockOptions *ForkedBlockOptions) (*core.BlockChain, *backends.SimulatedBackend, *types.Block, common.Address, func(account accounts.Account, hash []byte) ([]byte, error), *types.Block) {
	// Preparation
	var err error
	...
	engine := blockchain.Engine().(*BRDPoS.BRDPoS)
	...
	for i := 1; i <= numOfBlocks; i++ {
		...
		if int64(i) > chainConfig.BRDPoS.V2.SwitchBlock.Int64() {
			blockCoinBase = signer.Hex()
		}
		roundNumber := int64(i) - chainConfig.BRDPoS.V2.SwitchBlock.Int64()
		...
		// First v2 block
		if (int64(i) - chainConfig.BRDPoS.V2.SwitchBlock.Int64()) == 1 {
			lastv1BlockNumber := block.Header().Number.Uint64() - 1
			checkpointBlockNumber := lastv1BlockNumber - lastv1BlockNumber%chainConfig.BRDPoS.Epoch
...
// V2 concensus engine, compared to PrepareBRCTestBlockChainForV2Engine: (1) no forking (2) add penalty
func PrepareBRCTestBlockChainWithPenaltyForV2Engine(t *testing.T, numOfBlocks int, chainConfig *params.ChainConfig) (*core.BlockChain, *backends.SimulatedBackend, *types.Block, common.Address, func(account accounts.Account, hash []byte) ([]byte, error)) {
	// Preparation
	var err error
	signer, signFn, err := backends.SimulateWalletAddressAndSignFn()
	if err != nil {
	...
	// Authorise
	blockchain.Engine().(*BRDPoS.BRDPoS).Authorize(signer, signFn)
...
// V2 concensus engine, compared to PrepareBRCTestBlockChainForV2Engine: (1) no forking (2) 128 masternode candidates
func PrepareBRCTestBlockChainWith128Candidates(t *testing.T, numOfBlocks int, chainConfig *params.ChainConfig) (*core.BlockChain, *backends.SimulatedBackend, *types.Block, common.Address, func(account accounts.Account, hash []byte) ([]byte, error)) {
	// Preparation
	var err error
	signer, signFn, err := backends.SimulateWalletAddressAndSignFn()
	if err != nil {
...
func CreateBlock(blockchain *core.BlockChain, chainConfig *params.ChainConfig, startingBlock *types.Block, blockNumber int, roundNumber int64, blockCoinBase string, signer common.Address, signFn func(account accounts.Account, hash []byte) ([]byte, error), penalties []byte, signersKey []*ecdsa.PrivateKey, merkleRoot string) *types.Block {
	...
	if big.NewInt(int64(blockNumber)).Cmp(chainConfig.BRDPoS.V2.SwitchBlock) == 1 { // Build engine v2 compatible extra data field
		...
		if int64(blockNumber) == (chainConfig.BRDPoS.V2.SwitchBlock.Int64() + 1) { // This is the first v2 block, we need to copy the last v1 epoch master node list and inject into v2 validators
			// Get last master node list from last v1 block
			lastv1Block := blockchain.GetBlockByNumber(chainConfig.BRDPoS.V2.SwitchBlock.Uint64())
		...
		} else if roundNumber%int64(chainConfig.BRDPoS.Epoch) == 0 {
		...
		if big.NewInt(int64(blockNumber)).Cmp(chainConfig.BRDPoS.V2.SwitchBlock) == 0 || blockNumber%int(chainConfig.BRDPoS.Epoch) == 0 {
			// reset extra
			header.Extra = []byte{}
			...
			sighash, err := signFn(accounts.Account{Address: signer}, blockchain.Engine().(*BRDPoS.BRDPoS).SigHash(header).Bytes())
			if err != nil {
...
func findSignerAndSignFn(bc *core.BlockChain, header *types.Header, signer common.Address, signFn func(account accounts.Account, hash []byte) ([]byte, error), config *params.ChainConfig) (common.Address, func(account accounts.Account, hash []byte) ([]byte, error)) {
	...
	if header.Number.Cmp(config.BRDPoS.V2.SwitchBlock) > 0 {
		...
		index := uint64(round) % config.BRDPoS.Epoch % uint64(len(masterNodes))
...
func sealHeader(bc *core.BlockChain, header *types.Header, signer common.Address, signFn func(account accounts.Account, hash []byte) ([]byte, error)) {
	// Sign all the things and seal it
	signedBlockHeader := bc.Engine().(*BRDPoS.BRDPoS).SigHash(header)
...
func generateV2Extra(roundNumber int64, currentBlock *types.Block, signer common.Address, signFn func(account accounts.Account, hash []byte) ([]byte, error), accKeys []*ecdsa.PrivateKey) []byte {
	...
	gapNumber := currentBlock.Number().Uint64() - currentBlock.Number().Uint64()%params.TestBRDPoSMockChainConfig.BRDPoS.Epoch - params.TestBRDPoSMockChainConfig.BRDPoS.Gap
