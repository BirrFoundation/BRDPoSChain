...
func TestShouldVerifyBlock(t *testing.T) {
	...
	assert.Nil(t, err)
	// Enable verify
	config.BRDPoS.V2.SkipV2Validation = false
	// Block 901 is the first v2 block with round of 1
	blockchain, _, _, signer, signFn, _ := PrepareBRCTestBlockChainForV2Engine(t, 910, &config, nil)
...
func TestConfigSwitchOnDifferentCertThreshold(t *testing.T) {
	...
	config.BRDPoS.V2.SkipV2Validation = false
	// Block 901 is the first v2 block with round of 1
	blockchain, _, _, _, _, _ := PrepareBRCTestBlockChainForV2Engine(t, 915, &config, nil)
...
func TestConfigSwitchOnDifferentMasternodeCount(t *testing.T) {
	...
	config.BRDPoS.V2.SkipV2Validation = false
	// Block 901 is the first v2 block with round of 1
	blockchain, _, currentBlock, _, _, _ := PrepareBRCTestBlockChainForV2Engine(t, int(config.BRDPoS.Epoch)*2, &config, nil)
	adaptor := blockchain.Engine().(*BRDPoS.BRDPoS)
	x := adaptor.EngineV2
...
func TestShouldFailIfNotEnoughQCSignatures(t *testing.T) {
	...
	config.BRDPoS.V2.SkipV2Validation = false
	// Block 901 is the first v2 block with round of 1
	blockchain, _, currentBlock, signer, signFn, _ := PrepareBRCTestBlockChainForV2Engine(t, 902, &config, nil)
	adaptor := blockchain.Engine().(*BRDPoS.BRDPoS)
...
func TestShouldVerifyHeaders(t *testing.T) {
	...
	config.BRDPoS.V2.SkipV2Validation = false
	// Block 901 is the first v2 block with round of 1
	blockchain, _, _, _, _, _ := PrepareBRCTestBlockChainForV2Engine(t, 910, &config, nil)
	adaptor := blockchain.Engine().(*BRDPoS.BRDPoS)
...
func TestShouldVerifyHeadersEvenIfParentsNotYetWrittenIntoDB(t *testing.T) {
	...
	config.BRDPoS.V2.SkipV2Validation = false
	// Block 901 is the first v2 block with round of 1
	blockchain, _, block910, signer, signFn, _ := PrepareBRCTestBlockChainForV2Engine(t, 910, &config, nil)
	adaptor := blockchain.Engine().(*BRDPoS.BRDPoS)
	...
	blockNumber := 911
	roundNumber := int64(blockNumber) - config.BRDPoS.V2.SwitchBlock.Int64()
