...
func TestRewardBalance(t *testing.T) {
	contractBackend := backends.NewBRCSimulatedBackend(types.GenesisAlloc{
	...
	}, 42000000, params.TestBRDPoSMockChainConfig)
	...
	// validatorAddr, _, baseValidator, err := contract.DeployBRCValidator(transactOpts, contractBackend, big.NewInt(50000), big.NewInt(99), big.NewInt(100), big.NewInt(100))
	validatorCap := new(big.Int)
	validatorCap.SetString("50000000000000000000000", 10)
	validatorAddr, _, baseValidator, err := contractValidator.DeployBRCValidator(
		transactOpts,
		contractBackend,
		[]common.Address{addr},
...
func GetRewardBalancesRate(foudationWalletAddr common.Address, masterAddr common.Address, totalReward *big.Int, validator *contractValidator.BRCValidator) (map[common.Address]*big.Int, error) {
...
func GetCandidatesOwnerBySigner(validator *contractValidator.BRCValidator, signerAddr common.Address) common.Address {
	owner := signerAddr
	opts := new(bind.CallOpts)
...
func TestStatedbUtils(t *testing.T) {
	...
	contractBackend := backends.NewBRCSimulatedBackend(genesisAlloc, 10000000, params.TestBRDPoSMockChainConfig)
	transactOpts := bind.NewKeyedTransactor(key)
	...
	contractBackendForValidator := backends.NewBRCSimulatedBackend(genesisAlloc, 10000000, params.TestBRDPoSMockChainConfig)
