...
import (
	...
	"BRDPoSChain/contracts/trc21issuer"
	"BRDPoSChain/contracts/trc21issuer/simulation"
...
func airDropTokenToAccountNoBRC() {
...
func testTransferTRC21TokenWithAccountNoBRC() {
	client, err := ethclient.Dial(simulation.RpcEndpoint)
	if err != nil {
		fmt.Println(err, client)
	}

	// access to address which received token trc20 but dont have BRC
...
func main() {
	...
	for i := 0; i < 10000000; i++ {
		airDropTokenToAccountNoBRC()
		fmt.Println("Finish airdrop token to a account")
		testTransferTRC21TokenWithAccountNoBRC()
		fmt.Println("Finish transfer trc21 token with a account no BRC")
		testTransferTrc21Fail()
		fmt.Println("Finish testing ! Success transferAmount token trc20 with a account no BRC")
	}
