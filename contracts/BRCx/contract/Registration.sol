...
contract RelayerRegistration {
    ...
    address constant private BRCNative = 0x0000000000000000000000000000000000000001;
    ...
    AbstractBRCXListing private BRCXListing;
    ...
    constructor (address BRCxListing, uint maxRelayers, uint maxTokenList, uint minDeposit) public {
        BRCXListing = AbstractBRCXListing(BRCxListing);
        RelayerCount = 0;
    ...
    function register(address coinbase, uint16 tradeFee, address[] memory fromTokens, address[] memory toTokens) public payable {
        ...
        // check valid tokens, token must pair with BRC(x/BRC)
    ...
    function depositMore(address coinbase) public payable relayerOwnerOnly(coinbase) onlyActiveRelayer(coinbase) notForSale(coinbase) nonZeroValue {
        require(msg.value >= 1 ether, "At least 1 BRC is required for a deposit request");
    ...
    function indexOf(address[] memory BRCPair, address target) internal pure returns (bool){
        for (uint i = 0; i < BRCPair.length; i ++) {
            if (BRCPair[i] == target) {
                return true;
            }
        }
    ...
    function validateTokens(address[] memory fromTokens, address[] memory toTokens) internal returns(bool) {
        ...
        address[] memory BRCPairs = new address[](fromTokens.length);
        address[] memory nonBRCPairs = new address[](fromTokens.length);

        for (uint i = 0; i < toTokens.length; i++) {
            bool b = BRCXListing.getTokenStatus(toTokens[i]) || (toTokens[i] == BRCNative);
            b = b && (BRCXListing.getTokenStatus(fromTokens[i]) || fromTokens[i] == BRCNative);
            if (!b) {
                return false;
            }
            if (toTokens[i] == BRCNative) {
                BRCPairs[countPair] = fromTokens[i];
                countPair++;
            } else {
                if (fromTokens[i] == BRCNative) {
                    BRCPairs[countPair] = toTokens[i];
                    countPair++;
                }
                nonBRCPairs[countNonPair] = toTokens[i];
                countNonPair++;
            }
        }

        for (uint j = 0; j < countNonPair; j++) {
            if (!indexOf(BRCPairs, nonBRCPairs[j])) {
                return false;
            }
        }
        return true;
    ...
    function addingTokenValidation(
        ...
        address[] memory BRCPairs = new address[](RELAYER_LIST[coinbase]._toTokens.length + 1);

        bool b = BRCXListing.getTokenStatus(toToken) || (toToken == BRCNative);
        b = b && (BRCXListing.getTokenStatus(fromToken) || fromToken == BRCNative);
        if (!b) {
            return false;
        }

        if (fromToken == BRCNative || toToken == BRCNative) {
            return true;
        }

        // get tokens that paired with BRC
        for (uint i = 0; i < RELAYER_LIST[coinbase]._toTokens.length; i++) {
            if (RELAYER_LIST[coinbase]._toTokens[i] == BRCNative) {
                BRCPairs[countPair] = RELAYER_LIST[coinbase]._fromTokens[i];
                countPair++;
            } else {
                if (RELAYER_LIST[coinbase]._fromTokens[i] == BRCNative) {
                    BRCPairs[countPair] = RELAYER_LIST[coinbase]._toTokens[i];
                    countPair++;
                }
            }
        }
        if (!indexOf(BRCPairs, toToken)) {
            return false;
        }
        return true;
    }
