...
import (
	...
	simulation "BRDPoSChain/contracts/BRCx/testnet"
...
func main() {
	...
	trc21IssuerAddr, trc21Issuer, err := BRCx.DeployTRC21Issuer(auth, client, simulation.MinTRC21Apply)
	if err != nil {
	...
	//init BRCX Listing in
	auth.Nonce = big.NewInt(int64(nonce + 1))
	BRCxListtingAddr, BRCxListing, err := BRCx.DeployBRCXListing(auth, client)
	if err != nil {
		log.Fatal("DeployBRCXListing", err)
	}
	BRCxListing.TransactOpts.GasPrice = big.NewInt(250000000000000)

	fmt.Println("===> BRCx listing address", BRCxListtingAddr.Hex())
	fmt.Println("wait 10s to execute init smart contract : BRCx listing ")
	time.Sleep(2 * time.Second)

	// init Relayer Registration
	auth.Nonce = big.NewInt(int64(nonce + 2))
	relayerRegistrationAddr, relayerRegistration, err := BRCx.DeployRelayerRegistration(auth, client, BRCxListtingAddr, simulation.MaxRelayers, simulation.MaxTokenList, simulation.MinDeposit)
	if err != nil {
	...
	lendingRelayerRegistrationAddr, lendingRelayerRegistration, err := BRCx.DeployLendingRelayerRegistration(auth, client, relayerRegistrationAddr, BRCxListtingAddr)
	if err != nil {
	...
	applyBRCXListing(BRCxListing, tokenList, currentNonce)
	...
	nonce = nonce + 1
	lendingRelayerRegistration.TransactOpts.Nonce = big.NewInt(int64(nonce))
	_, err = lendingRelayerRegistration.AddCollateral(simulation.BRCNative, simulation.CollateralDepositRate, simulation.CollateralLiquidationRate, simulation.CollateralRecallRate)
	...
	nonce = nonce + 1
	lendingRelayerRegistration.TransactOpts.Nonce = big.NewInt(int64(nonce))
	_, err = lendingRelayerRegistration.AddBaseToken(simulation.BRCNative)
	if err != nil {
		log.Fatal("Lending add base token BRC", err)
	}
	...
	relayerRegistration, err = BRCx.NewRelayerRegistration(ownerRelayer, relayerRegistrationAddr, client)
	if err != nil {
	...
	/*
		for _, token := range tokenList {
			...
			toTokens = append(toTokens, simulation.BRCNative)
		}
	*/

	// BRC/BTC
	fromTokens = append(fromTokens, simulation.BRCNative)
	...
	fmt.Println("wait 2s to apply token to list BRCx")
	time.Sleep(2 * time.Second)

	// Lending apply
	nonce = nonce + 1
	lendingRelayerRegistration, err = BRCx.NewLendingRelayerRegistration(ownerRelayer, lendingRelayerRegistrationAddr, client)
	if err != nil {
		log.Fatal("NewRelayerRegistration", err)
	}
	lendingRelayerRegistration.TransactOpts.Nonce = big.NewInt(int64(nonce))
	...
	// BRC 1 min
	baseTokens = append(baseTokens, simulation.BRCNative)
	terms = append(terms, big.NewInt(60))
	collaterals = append(collaterals, common.HexToAddress("0x0"))
...
func initTRC21(auth *bind.TransactOpts, client *ethclient.Client, nonce uint64, tokenNameList []string) []map[string]interface{} {
	...
	for _, tokenName := range tokenNameList {
		...
		if tokenName == "ADA" {
			d = 0
			tokenCap = new(big.Int).Div(simulation.TRC21TokenCap, simulation.BaseBRC)
		}
		if tokenName == "USDT" {
			d = 6
		...
		tokenAddr, _, err := BRCx.DeployTRC21(auth, client, simulation.Owners, simulation.Required, tokenName, tokenName, d, tokenCap, simulation.TRC21TokenFee, depositFee, withdrawFee)
		if err != nil {
...
func applyIssuer(trc21Issuer *BRCx.TRC21Issuer, tokenList []map[string]interface{}, nonce uint64) {
	for _, token := range tokenList {
...
func applyBRCXListing(BRCxListing *BRCx.BRCXListing, tokenList []map[string]interface{}, nonce uint64) {
	for _, token := range tokenList {
		BRCxListing.TransactOpts.Nonce = big.NewInt(int64(nonce))
		BRCxListing.TransactOpts.Value = simulation.BRCXListingFee
		_, err := BRCxListing.Apply(token["address"].(common.Address))
		if err != nil {
			log.Fatal("BRCxListing Apply ", token["name"].(string), err)
		}
		fmt.Println("applyBRCXListing ", token["name"].(string))
		nonce = nonce + 1
	}
	time.Sleep(5 * time.Second)
}
...
func airdrop(auth *bind.TransactOpts, client *ethclient.Client, tokenList []map[string]interface{}, addresses []common.Address, nonce uint64) {
	for _, token := range tokenList {
		for _, address := range addresses {
			trc21Contract, _ := BRCx.NewTRC21(auth, token["address"].(common.Address), client)
