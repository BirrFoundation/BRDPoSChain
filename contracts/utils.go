...
import (
	...
	"BRDPoSChain/contracts/blocksigner/contract"
	randomizeContract "BRDPoSChain/contracts/randomize/contract"
...
func CreateTransactionSign(chainConfig *params.ChainConfig, pool *txpool.TxPool, manager *accounts.Manager, block *types.Block, chainDb ethdb.Database, eb common.Address) error {
	...
	if chainConfig.BRDPoS != nil {
		...
		checkNumber := blockNumber % chainConfig.BRDPoS.Epoch
		...
		if !exist && checkNumber > 0 && common.EpocBlockSecret <= checkNumber && common.EpocBlockOpening > checkNumber {
			...
			tx, err := BuildTxSecretRandomize(nonce+1, common.RandomizeSMCBinary, chainConfig.BRDPoS.Epoch, randomizeKeyValue)
			if err != nil {
...
func GetRandomizeFromContract(client bind.ContractBackend, addrMasternode common.Address) (int64, error) {
	randomize, err := randomizeContract.NewBRCRandomize(common.RandomizeSMCBinary, client)
	if err != nil {
...
func GetRewardForCheckpoint(c *BRDPoS.BRDPoS, chain consensus.ChainReader, header *types.Header, rCheckpoint uint64, totalSigner *uint64) (map[common.Address]*RewardLog, error) {
